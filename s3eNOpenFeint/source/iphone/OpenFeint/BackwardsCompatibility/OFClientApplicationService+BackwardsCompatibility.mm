...
@implementation OFClientApplicationService (BackwardsCompatibility)
...
{
    [self getPlayedGamesForUser:userId withPage:pageIndex andCountPerPage:perPage 
...
{
    [self getFavoriteGamesForUser:userId withPage:pageIndex andCountPerPage:perPage 
              onSuccessInvocation:onSuccess.getInvocation() 
              onFailureInvocation:onFailure.getInvocation()];
}
...
{
    [self getPlayedGamesForLocalUsersFriends:pageIndex 
                         onSuccessInvocation:onSuccess.getInvocation() 
                         onFailureInvocation:onFailure.getInvocation()];
}
...
{
    [self getGameProfilePageComparisonInfo:clientApplicationId comparedToUserId:comparedToUserId 
                       onSuccessInvocation:onSuccess.getInvocation() 
                       onFailureInvocation:onFailure.getInvocation()];
}
...
{
    [self getGameProfilePageComparisonInfo:clientApplicationId comparedToUserId:nil 
                       onSuccessInvocation:onSuccess.getInvocation() 
                       onFailureInvocation:onFailure.getInvocation()];
}
...
{
    [self getPlayerReviewForGame:clientApplicationId byUser:userId 
             onSuccessInvocation:onSuccess.getInvocation() 
             onFailureInvocation:onFailure.getInvocation()];
}
