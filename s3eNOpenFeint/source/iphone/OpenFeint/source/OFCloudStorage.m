...
+ (OFRequestHandle*)upload:(NSData*)data withKey:(NSString*)key
{
	OFRequestHandle* handle = [OFCloudStorageService uploadBlob:data
														withKey:key
                                            onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_uploadSuccess)]
                                            onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_uploadFail:)]];
...
//															 onFailure:OFDelegate(self, @selector(_downloadKeysForCurrentUserFail))];
	OFRequestHandle* handle = [OFCloudStorageService getIndexOnSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_downloadKeysForCurrentUserSuccesss:)] 
                                                             onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_downloadKeysForCurrentUserFail)]];											
	[OFRequestHandlesForModule addHandle:handle forModule:[OFCloudStorage class]];
	return handle;
}
...
+ (OFRequestHandle*)downloadDataWithKey:(NSString*)key
{
	OFRequestHandle* handle = [OFCloudStorageService downloadBlobWithKey:key
                                                     onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_downloadSuccess:)]
                                                     onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_downloadFail:)]];
