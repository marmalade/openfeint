...
+ (OFRequestHandle*)submitDeferredAchievements
{
	...
	handle = [OFAchievementService 
              submitQueuedUpdateAchievementsInvocation:[OFInvocation invocationForTarget:self selector:@selector(_submitDeferredSuccess)]
              onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_submitDeferredFailure)]];
...
- (OFRequestHandle*)updateProgressionComplete:(double)updatePercentComplete andShowNotification:(BOOL)showUpdateNotification
{
	...
	handle = [OFAchievementService updateAchievement:self.resourceId
                                 ...
                                 onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_updateProgressionSuccess)]
                                 onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_updateProgressionFailure)]];
...
- (OFRequestHandle*)getIcon
{
    ...
    if(sharedDelegate)
    {
        if([sharedDelegate respondsToSelector:@selector(didGetIcon:OFAchievement:)])
        {
            success = [OFInvocation invocationForTarget:sharedDelegate selector:@selector(didGetIcon:OFAchievement:) userParam:self];
        }
        
        if([sharedDelegate respondsToSelector:@selector(didFailGetIconOFAchievement:)])
        {
            failure = [OFInvocation invocationForTarget:sharedDelegate selector:@selector(didFailGetIconOFAchievement:) userParam:self];
        }
    }
    OFRequestHandle* handle = [OpenFeint getImageFromUrl:iconUrl forModule:[OFAchievement class] onSuccess:success onFailure:failure];
	return handle;
}
...
- (id)initWithLocalSQL:(OFSqlQuery*)queryRow
{
	...
	if (self != nil)
	{	
		title = [[queryRow stringValue:@"title"] retain];
		description = [[queryRow stringValue:@"description"]retain];
		gamerscore = [queryRow intValue:@"gamerscore"];
		iconUrl = [[queryRow stringValue:@"icon_file_name"] retain];
		isSecret = [queryRow boolValue:@"is_secret"];
		position = [queryRow intValue:@"position"];
		...
		int64_t secondsSince1970 = [queryRow int64Value:@"unlocked_date"];
...
[OFResourceField fieldSetter:@selector(setDescription:)], @"description",
[OFResourceField fieldSetter:@selector(setGamerscoreFromString:)], @"gamerscore",
[OFResourceField fieldSetter:@selector(setIconUrl:)], @"icon_url",
[OFResourceField fieldSetter:@selector(setIsSecretFromString:)], @"is_secret",
[OFResourceField fieldSetter:@selector(setPercentComplete:)], @"percent_complete",
[OFResourceField fieldSetter:@selector(setIsUnlockedByComparedToUserFromString:)], @"is_unlocked_by_compared_to_user",
[OFResourceField fieldSetter:@selector(setComparedToUserId:)], @"compared_to_user_id",
[OFResourceField fieldSetter:@selector(setUnlockDateFromString:)], @"unlock_date",
[OFResourceField fieldSetter:@selector(setPositionFromString:)], @"position",
	...
	if([OpenFeint isLoggedIntoGameCenter])
	{
		...
		else
		{
			...
			[submitObject submitToGameCenterOnlyWithIds:submitAchievementIds
                                    ...
                                    onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_forceSyncGameCenterSuccess)]
                                    onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_forceSyncGameCenterFailure)]];
