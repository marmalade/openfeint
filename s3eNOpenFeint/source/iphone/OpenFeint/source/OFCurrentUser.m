...
+ (OFRequestHandle*)favoriteCurrentGame:(NSString*)reviewText
{
	...
	handle = [OFClientApplicationService makeGameFavorite:[OpenFeint clientApplicationId] 
                                      ...
                                      onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_favoriteCurrentGameSuccess)]
                                      onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_favoriteCurrentGameFailure)]];
...
+ (OFRequestHandle*)sendFriendRequest:(OFUser*)user
{
	...
	handle = [OFFriendsService makeLocalUserFollow:user.resourceId 
                               onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_currentUserSendFriendRequestSuccess)]
                               onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_currentUserSendFriendRequestFailure)]];
...
+ (OFRequestHandle*)unfriend:(OFUser*)user
{
	...
	handle = [OFFriendsService makeLocalUserStopFollowing:user.resourceId
                                      onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_currentUserUnfriendSuccess)]
                                      onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_currentUserUnfriendFailure)]];
...
//                                                         onFailure:OFDelegate(self, @selector(_checkConnectedToSocialNetworkFailure))
	handle = [OFUsersCredentialService getIndexOnSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_checkConnectedToSocialNetworkSuccess:)]
											  onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_checkConnectedToSocialNetworkFailure)]
...
{
	return [self unviewedChallengesCount] + [OpenFeint pendingFriendsCount] + [OpenFeint unreadInboxTotal] + [OpenFeint unreadAccouncementCount];
}
