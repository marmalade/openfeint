...
{
	OFAssert(leaderboard.descendingScoreOrder, @"%@ uses ascending sort order. Only descending sort order supported for distributed scores", leaderboard.name);
		...
		newCurrentPage > 0)
    {
		...
		else
		{
			self.activeRequest = [OFHighScoreService 
                                  ...
                                  onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_downloadFinished:)]
                                  onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_downloadFailed)]];
