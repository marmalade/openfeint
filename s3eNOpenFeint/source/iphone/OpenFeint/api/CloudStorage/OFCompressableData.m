...
#define ZLIB_MAGIC_SIZE 8
#define ZLIB_MAGIC_HEADER_SIZE ZLIB_MAGIC_SIZE + sizeof(size_t)
static const char* sZlibHeader = "OFZLHDR0";


static BOOL sVerbose = NO;
static BOOL sDisableCompression = YES;
...
-(void) compress {
    ...
    if(self.data && [self.data length]) {
        ...
        if(error == Z_OK) {
            if(destSize + ZLIB_MAGIC_HEADER_SIZE < oldSize) {  //TODO: make this more strict?
                self.data = [NSData dataWithBytes:destBuffer length:destSize];
                self.uncompressedSize = oldSize;
                if(sVerbose) {
                    OFLog(@"ZLIB: Compression ratio for %lu/%d is %d%%", (destSize + ZLIB_MAGIC_HEADER_SIZE), (int)oldSize, (int)((destSize + ZLIB_MAGIC_HEADER_SIZE)*100.f / self.uncompressedSize + 0.5f));
                }
            }
            else {
                if(sVerbose)
                    OFLog(@"ZLIB: Rejecting compression %lu/%d", destSize + ZLIB_MAGIC_HEADER_SIZE, (int)oldSize);
            }
        }
        else {
...
-(NSData*) internalUncompressedData {
...
#ifndef OF_EXCLUDE_ZLIB
    unsigned char *destBuffer = (unsigned char *)malloc(self.uncompressedSize);
    ...
    if(error == Z_OK) {        
        returnVal = [NSData dataWithBytes:destBuffer length:destSize];
    }
    else {
...
-(NSData*) serializedData {
    ...
    if(size > 0) {    
        NSMutableData *retData = [NSMutableData dataWithBytes:sZlibHeader length:ZLIB_MAGIC_SIZE];
...
+(OFCompressableData*) dataWithSerializedData:(NSData*)_data {
    ...
    char header[ZLIB_MAGIC_SIZE];    
    header[ZLIB_MAGIC_SIZE - 1] = 'x';  //if the data doesn't have 8 characters in it, then the header won't have anything copied into it, putting this here to avoid false positives
    [_data getBytes:&header length:ZLIB_MAGIC_SIZE];
    if(!memcmp(header, sZlibHeader, ZLIB_MAGIC_SIZE)) {
        size_t size;
        [_data getBytes:&size range:NSMakeRange(ZLIB_MAGIC_SIZE,sizeof(size_t))];
        decompressor.uncompressedSize = size;
        decompressor.data = [NSData dataWithBytes:(char*)[_data bytes] + ZLIB_MAGIC_HEADER_SIZE length:[_data length]-ZLIB_MAGIC_HEADER_SIZE];
