...
- (void) addParamToData:(NSMutableData*)multipartData boundary:(NSData*)boundary paramName:(NSString*)paramName paramValue:(NSString*)paramValue
{
	...
	NSString* headerSpacer = [self getHeaderSpacer];
...
- (NSData*)createS3HttpBodyForBlob:(NSData*)blob
{
	...
	NSData* boundary = [self getBoundaryMarkerStart];
	NSData* boundaryEnd = [self getBoundaryMarkerEnd];
	...
	NSString* headerContentDisposition = [NSString stringWithFormat:@"Content-Disposition: form-data; name=\"file\"; filename=\"test.blob\"\r\n"];
	NSString* headerContentType = @"Content-Type: \"application/octet-stream\"\r\n";
	NSString* headerSpacer = [self getHeaderSpacer];
...
[OFResourceField fieldSetter:@selector(setAction:)], @"action",
[OFResourceField fieldSetter:@selector(setKey:)], @"key",
[OFResourceField fieldSetter:@selector(setAWSAccessKeyId:)], @"AWSAccessKeyId",
[OFResourceField fieldSetter:@selector(setAcl:)], @"acl",
[OFResourceField fieldSetter:@selector(setPolicy:)], @"policy",
[OFResourceField fieldSetter:@selector(setSignature:)], @"signature",
        nil] retain];
    }
