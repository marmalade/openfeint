...
NSString* OpenFeintControllerSelectProfilePicture = @"SelectProfilePicture";
...
+ (void)launchDashboardWithSelectProfilePicturePage
{
	[OpenFeint launchDashboardWithDelegate:nil tabControllerName:nil andController:OpenFeintControllerSelectProfilePicture];
}
...
+ (void)launchDashboardWithListGlobalChatRoomsPage
{
	...
	{
		OFSelectChatRoomDefinitionController* chatController = (OFSelectChatRoomDefinitionController*)[[OFControllerLoaderObjC loader] load:@"SelectChatRoomDefinition"];// load(@"SelectChatRoomDefinition");
...
{
	NSArray* controllers = [NSArray arrayWithObject:[[OFControllerLoaderObjC loader] load:@"UseNewOrOldAccount"]];// load(@"UseNewOrOldAccount")];
...
+ (void)launchDashboardWithForumsPage
{
	if ([OpenFeint allowUserGeneratedContent])
	{
		NSArray* controllers = [NSArray arrayWithObject:[[OFControllerLoaderObjC loader] load:@"ForumTopicList"]];// load(@"ForumTopicList")];
		[OpenFeint launchDashboardWithDelegate:nil tabControllerName:OpenFeintDashBoardTabNowPlaying andControllers:controllers];
	}
	else
	{
...
{
	[[OFControllerLoaderObjC loader] loadAndLaunch:@"SelectInviteType" withParams:nil];
}

+ (void)launchDashboardWithSpecificInvite:(NSString*)inviteIdentifier
{
	NSDictionary* params = [NSDictionary dictionaryWithObjectsAndKeys:
							inviteIdentifier, @"invite_id",
							nil];
	
	[[OFControllerLoaderObjC loader] loadAndLaunch:@"SelectInviteType" withParams:params];
}
...
+ (void)launchDashboardWithMoreGamesPage
{
	NSDictionary* params = [NSDictionary dictionaryWithObjectsAndKeys:
							@"target_service_index", @"scope",
							@"developers_picks", @"target_discovery_page_name",
							OFLOCALSTRING(@"More Games"), @"title",
							nil];

	[[OFControllerLoaderObjC loader] loadAndLaunch:@"PlayedGame" withParams:params];
}
