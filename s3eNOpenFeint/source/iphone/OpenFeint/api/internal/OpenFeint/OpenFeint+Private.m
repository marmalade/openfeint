...
static OpenFeint* gInstance = nil;
static unsigned int kOverlayBackgroundViewTag = 489;
...
{
	const BOOL isAutomaticallyPrompting = shouldShowDashboard == NO;
...
{
	[self launchLoginFlowAndShowDashboard:NO withOptionalRequest:nil];
}

+ (void)launchLoginFlowToDashboard
{
	[self launchLoginFlowAndShowDashboard:YES withOptionalRequest:nil];
}
...
- (void)session:(OFSession*)session didLoginUser:(OFUser*)user previousUser:(OFUser*)previousUser
{
	...
	mSuccessfullyBootstrapped = YES;
	
	s_BootstrapCompleted = YES;
	
    for(OFInvocation* invocation in mBootstrapSuccessInvocations)
    {
        [invocation invoke];
    }
    [mBootstrapSuccessInvocations removeAllObjects];
	...
	if (disabled || [OpenFeint doneWithGetTheMost]) {
		...
		getTheMostController = (UIViewController*)[[OFControllerLoaderObjC loader] load:@"LinkSocialNetworks"];// load(@"LinkSocialNetworks");
...
{
    NSString* userId = [[OpenFeint lastLoggedInUserId] retain];
	...
	OF_OPTIONALLY_INVOKE_DELEGATE_WITH_PARAMETER([OpenFeint getDelegate], userLoggedOut:, (id)userId);
    [userId release];
}
...
- (void)session:(OFSession*)session failureWithException:(OFServerException*)exception
    ...
    for(OFInvocation* invocation in mBootstrapFailureInvocations)
    {
        [invocation invoke];
    }
    [mBootstrapFailureInvocations removeAllObjects];
	[self _invokeLoggedInDelegate];
...
+ (void)doBootstrapAsUserId:(NSString*)userId newUser:(BOOL)createNewAccount onSuccess:(OFInvocation*)chainedOnSuccess onFailure:(OFInvocation*)chainedOnFailure
{
	...
	{
		[OpenFeint addBootstrapInvocations:chainedOnSuccess onFailure:chainedOnFailure];
...
{
	OpenFeint* instance = [OpenFeint sharedInstance];
    //trap if anyone is passing a nil delegate
    if(onSuccess)
        [instance->mBootstrapSuccessInvocations addObject:onSuccess];
    if(onFailure)
        [instance->mBootstrapFailureInvocations addObject:onFailure];
}
...
+ (void)presentConfirmAccountModalInvocation:(OFInvocation*)onCompletionInvocation useModalInDashboard:(BOOL)useModalInDashboard
{
	...
	OFDoBootstrapController* approvalFlow = (OFDoBootstrapController*)[[OFControllerLoaderObjC loader] load:@"DoBootstrap"];// load(@"DoBootstrap");
    approvalFlow.onCompletionInvocation = onCompletionInvocation;
...
+ (BOOL)tryCreateDashboardWithModal:(UIViewController*)modal
{
	...
	{
		instance->mOFRootController = (OFRootController*)[[[OFControllerLoaderObjC loader] load:@"Root"] retain]; // load(@"Root") retain];
	}
...
+ (void)presentModalOverlay:(UIViewController*)modal opaque:(BOOL)isOpaque
{
	...
	{
		instance->mIsQueuedModalAnOverlay = YES;
		return;
	}

	instance->mIsShowingOverlayModal = YES;
...
+ (void)presentRootControllerWithModal:(UIViewController*)modal
{
	...
	if(![self tryCreateDashboardWithModal:modal])
	{
		instance->mIsQueuedModalAnOverlay = NO;
		return;
	}

	instance->mIsShowingOverlayModal = NO;
...
+ (void)presentRootControllerWithTabbedDashboard:(NSString*)controllerName pushControllers:(NSArray*)pushControllers
{
	...
	{
        NSString * s = @"TabbedDashboard";
		tabController = (OFTabbedDashboardController*)[[OFControllerLoaderObjC loader] load:s];// load(@"TabbedDashboard");
	}
		...
		if (allowed)
		{
			if ([obj isKindOfClass:[NSString class]])
			{
				if (controllerToPush && [controllerToPush isKindOfClass:[OFLeaderboardController class]])
				{
					...
					if( leaderboard )
					{
						controllerToPush = (UIViewController*)[[OFControllerLoaderObjC loader] load:OpenFeintControllerHighScores];// load(OpenFeintControllerHighScores);
					...
					controllerToPush = (UIViewController*)[[OFControllerLoaderObjC loader] load:pushControllerName];// load(pushControllerName);
				}
			}
			else if ([obj isKindOfClass:[UIViewController class]])
			{
				...
				OFAssert(NO, @"Unrecognized argument type!");
...
//		OFDelegate presentRootControllerDelegate([OpenFeint sharedInstance], @selector(_presentRootControllerWithTabbedDashboard));
		[OpenFeint presentUserFeintApprovalModalInvocation:[OFInvocation invocationForTarget:OpenFeint.sharedInstance selector:@selector(_presentRootControllerWithTabbedDashboard)] 
	...
	else if (([instance->mProvider isAuthenticated] && [OpenFeint lastLoggedInUserId])) 
	{
        // If the user hasn't yet approved OF, this will launch the "get more out of your game" screen somewhere.
        // This code has been modified to not allow that, but we want to completely remove the screen in the future.
...
+ (void)dismissRootController
{
	...
	instance->mIsDashboardDismissing = YES;
	
	if(instance->mIsShowingOverlayModal)
	{
		UIView* backgroundView = [instance->mOFRootController.view findViewByTag:kOverlayBackgroundViewTag];
		[UIView beginAnimations:nil context:nil];
		[UIView setAnimationDuration:0.5f];
		backgroundView.alpha = 0.0f;
		[UIView commitAnimations];
...
+ (void)displayUpgradeRequiredErrorMessage:(NSData*)data
{
	if([OpenFeint isShowingFullScreen])
	{
        OFXmlElement* errorDocument = [OFXmlElement elementWithData:data];
        NSString* message = [errorDocument getChildWithName:@"error_message"].value;
...
+ (void)displayErrorMessage:(NSString*)message
{
	...
	{
		UINavigationController* activeNavController = [self getActiveNavigationController];
		if (![OpenFeint isDashboardHubOpen] || OFReachability.isConnectedToInternet)
		{
...
{	
	...
	[OpenFeint postDashboardWillAppear];
...
+ (void)dashboardWillDisappear
{
	...
	[OpenFeint postDashboardWillDisappear];
...
+ (void)setupOfflineDatabase
{
    ...
    if([[OpenFeint clientApplicationId] length] > 0 && ![[OpenFeint clientApplicationId] isEqualToString:@"0"])
    {
        dbName = [NSString stringWithFormat:@"feint_offline_%@", [OpenFeint clientApplicationId]];
    }
    
	if (SQLITE_OK != sqlite3_open([[OFSettings savePathForFile:dbName] UTF8String], &[OpenFeint sharedInstance]->mOfflineDatabaseHandle))
	{
		OFAssert(NO, @"Failed to initialize offline database!");
		[OpenFeint sharedInstance]->mOfflineDatabaseHandle = nil;
	}
	if (SQLITE_OK != sqlite3_open([[OFSettings savePathForFile:dbName] UTF8String], &[OpenFeint sharedInstance]->mBootstrapOfflineDatabaseHandle))
	{
		OFAssert(NO, @"Failed to initialize offline database!");
...
+ (BOOL)developerAllowsUserGeneratedContent
{
    ...
    return 	!instance->mDeveloperDisabledUGC;
}
...
{
    static NSString* gsdi = nil;
    if (!gsdi)
    {
        NSString* prehashed = [NSString stringWithFormat:@"OFGSDI.%@.%@", [OpenFeint clientApplicationId], [OpenFeint uniqueDeviceId]];
        gsdi = [[prehashed sha1] retain];
    }
    return gsdi;
}

+ (void)_imageLoadingSuccess:(OFASIHTTPRequest*) request
{
    [OFRequestHandlesForModule completeRequest:request];
    UIImage* image = [UIImage imageWithData:request.responseData];
    if(image)        
    {
		[[OFImageCache sharedInstance] store:image withIdentifier:[request.userInfo objectForKey:@"url"]];
        [[request.userInfo objectForKey:@"onSuccess"] invokeWith:image];
    }
    else 
	{
        [[request.userInfo objectForKey:@"onFailure"] invokeWith:image];
	}
}

+ (void)_imageLoadingFailed:(OFASIHTTPRequest*) request
{	
    [OFRequestHandlesForModule completeRequest:request];
    [[request.userInfo objectForKey:@"onFailure"] invoke];
}
...
+ (OFRequestHandle*)getImageFromUrl:(NSString *)url forModule:(id)module onSuccess:(OFInvocation*)success onFailure:(OFInvocation*)failure
{
    ...
    UIImage* image = [[OFImageCache sharedInstance] fetch:url];
    if(image)
    {
        [success invokeWith:image];
        return nil;
	}

    OFASIHTTPRequest* request = [OFASIHTTPRequest requestWithURL:[NSURL OFURLWithString:url]];
    request.delegate = self;
    request.didFinishSelector = @selector(_imageLoadingSuccess:);
    request.didFailSelector = @selector(_imageLoadingFailed:);
    
    NSMutableDictionary* userInfo = [NSMutableDictionary dictionaryWithObject:url forKey:@"url"];
    if (success) { [userInfo setObject:success forKey:@"onSuccess"]; }
    if (failure) { [userInfo setObject:failure forKey:@"onFailure"]; }
    request.userInfo = userInfo;
    
    [request startAsynchronous];
    
    OFRequestHandle* handle = [OFRequestHandle requestHandle:request];
    if(handle)
    {
        [OFRequestHandlesForModule addHandle:handle forModule:module];
    }
...
{
	if(newStatus != OFReachability_Not_Connected && [OpenFeint hasUserApprovedFeint])
	{
...
{
    return gInstance && gInstance->mApprovalScreenOpen;
}
