...
- (unsigned)readLengthForTerm
{
	...
	while(i < bytesDone)
	{
		const UInt8* subBuffer = (const UInt8*)[buffer bytes] + i;
		
		if(memcmp((const void*)subBuffer, [term bytes], j) == 0)
		{
...
- (CFIndex)searchForTermAfterPreBuffering:(CFIndex)numBytes
{
	...
	while((CFIndex)(i + [term length]) <= bytesDone)
	{
		const UInt8 *subBuffer = (const UInt8*)[buffer bytes] + i;
		
		if(memcmp((const void*)subBuffer, [term bytes], [term length]) == 0)
		{
				...
				if(theCurrentRead->term != nil)
				{
					if(didPreBuffer)
					{
						...
						if(overflow > 0)
						{
							...
							const UInt8* overflowBuffer = (const UInt8*)[buffer bytes] + theCurrentRead->bytesDone - overflow;
							
							[partialReadBuffer appendBytes:(const void*)overflowBuffer length:overflow];
					...
					else
					{
						...
						{
							const UInt8 *buf = (const UInt8 *)[theCurrentRead->buffer bytes] + (theCurrentRead->bytesDone - termlen);
							const UInt8 *seq = (const UInt8 *)[theCurrentRead->term bytes];
