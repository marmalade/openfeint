...
- (void) populateKnownResourceMap:(NSMutableDictionary*)namedResourceMap
{
	...
	[namedResourceMap setObject:[OFPresenceQueue class] forKey:[OFPresenceQueue getResourceName]];
}
...
- (void)_presenceMain:(id)ignored
{
    ...
    OFCRVStompClient* client = [[OFCRVStompClient alloc] 
                                initWithHost:[[OFSettings instance] getSetting:@"presence-host"] 
                                port:61613 
    ...
    [client release];  //not using autorelease since that won't be called until the presence ends
...
withSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_onPresenceQueueLoaded:)]
withFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_onFailedToLoadPresenceQueue)]
...
- (void)stompClient:(OFCRVStompClient *)theStompClient messageReceived:(NSString *)body withHeader:(NSDictionary *)messageHeader
{
	...
	if ([messageHeader objectForKey:@"X-HTTP-REQUEST"]) {
		...
		OFXmlElement *doc = [OFXmlElement elementWithString:[body stringByReplacingOccurrencesOfString:stringWithNUL withString:@""]];
		OFPaginatedSeries *resources = [OFResource resourcesFromXml:doc withMap:self.knownResources];
