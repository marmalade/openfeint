...
{
	FOR_EACH_ADDON_DO(id addonid = addon; if ([addonid respondsToSelector:@selector(preInitializeAddOn:)]) {[addon preInitializeAddOn:settings];})
}
...
+ (void)loadAddOnSettings:(OFSettings*)settings fromParser:(OFSettingsParser*)parser 
{
	FOR_EACH_ADDON_DO(id addonid = addon; if ([addonid respondsToSelector:@selector(loadSettings:fromParser:)]) {[addon loadSettings:settings fromParser:parser];})
}
...
{
	FOR_EACH_ADDON_DO(if([(id)addon respondsToSelector:@selector(userLoggedIn)]) [addon userLoggedIn])
}

+ (void)notifyAddOnsUserLoggedInPostIntro
{
	FOR_EACH_ADDON_DO(if([(id)addon respondsToSelector:@selector(userLoggedInPostIntro)]) [addon userLoggedInPostIntro])
}

+ (void)notifyAddOnsOfflineUserLoggedInPostIntro
{
	FOR_EACH_ADDON_DO(if([(id)addon respondsToSelector:@selector(offlineUserLoggedInPostIntro)]) [addon offlineUserLoggedInPostIntro])
}
