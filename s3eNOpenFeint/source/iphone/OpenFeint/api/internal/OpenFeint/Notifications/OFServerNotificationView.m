...
+ (void)showServerNotification:(OFServerNotification*)serverNotification inView:(UIView*)containerView
{
	...
	OFServerNotificationView* view = (OFServerNotificationView*)[[OFControllerLoaderObjC loader] loadView:self.notificationViewName]; // loadView([self notificationViewName]);
...
- (void) _loadImagesWithNotification:(OFServerNotification*)serverNotification
{
	...
	static NSString* DEFAULT_BACKGROUND = @"OFNotificationBackground.png";
	static NSString* DEFAULT_STATUS_INDICATOR = @"OpenFeintStatusIconNotificationFailure.png";
	static NSString* DEFAULT_ICON = @"OFProfileIconDefaultSelf.png";
...
- (void) _updateText:(OFServerNotification*)serverNotification
{
	...
	//Just Hide all to start at turn on appropriate fields.
	notice.hidden = YES;
			...
			andDesiredImage:(NSString*)desiredImageUrl
{
	...
	if(defaultImageName || desiredImageUrl)
	{
		...
		if(desiredImageUrl && ![desiredImageUrl isEqualToString:@""])
		{
            ...
            imageView.imageDownloadInvocation = [OFInvocation invocationForTarget:self selector:@selector(_imageFinishedDownloading)];
...
- (void)_imageFinishedDownloading
{
	...
	if(miDownloadsDone == miDownloadsNeeded)
	{
		if(!settingUpView)
		{
            ...
            backgroundImage.imageDownloadInvocation = nil;
            statusIndicator.imageDownloadInvocation = nil;
