...
{
	return (OFNotificationInputResponse*)[[[OFInputResponsePushController alloc] initWithControllerName:controllerName andNavController:navController andShowAsModal:showAsModal] autorelease];
}

- (void)respondToInput
{
	UIViewController* controllerToPush = [[OFControllerLoaderObjC loader] load:mControllerName]; // load(mControllerName);
	if (controllerToPush)
	{
		if (mShowAsModal)
		{
