...
{
	OFAchievementNotificationView* view = (OFAchievementNotificationView*)[[OFControllerLoaderObjC loader] loadView:[self notificationViewName]];// loadView([self notificationViewName]);
...
- (void)_iconFinishedDownloading
{
    ...
    notificationImage.imageDownloadInvocation = nil;
}
...
{	
    ...
    OFInvocation* showAndDismiss = [OFInvocation invocationForTarget:self selector:@selector(_iconFinishedDownloading)];
	...
	if (notificationData.unlockedAchievement)
	{
		...
		{
            notificationImage.imageDownloadInvocation = showAndDismiss;
