...
- (void)showBackgroundNoticeForLoader:(MPOAuthAPIRequestLoader*)request withNotice:(OFNotificationData*)noticeData
{
	...
	if ([self allowNotice:noticeData])
	{
		...
		if (topView)
		{
			...
			[OFNotificationView showNotificationWithRequest:request andNotice:noticeData.notificationText inView:topView];
		}
	}
	else
	{
...
- (void)showBackgroundNotice:(OFNotificationData*)noticeData andStatus:(OFNotificationStatus*)status
{
	if (![OpenFeint isDashboardHubOpen])
	{
		if ([self allowNotice:noticeData])
		{
			...
			{
				[self onNotificationWillShow:noticeData];
				[OFNotificationView showNotificationWithText:noticeData.notificationText andImageNamed:noticeData.imageName andStatus:status inView:topView];
			}
		}
		else
		{
...
{
	OFUnlockedAchievementNotificationData* noticeData = [OFUnlockedAchievementNotificationData dataWithAchievement:unlockedAchievement andPercentComplete:percentComplete];
	if ([self allowNotice:noticeData])
	{
		UIView* topView = [OpenFeint getTopApplicationWindow];
		if (topView)
		{
			[self onNotificationWillShow:noticeData];
			[OFAchievementNotificationView showAchievementNotice:noticeData inView:topView];
		}
	}
	else
	{
...
- (void)showChallengeNotice:(OFChallengeToUser*)challengeToUser
{
	...
	if ([self allowNotice:noticeData])
	{
		...
		{
			[self onNotificationWillShow:noticeData];
			[OFChallengeNotificationView showChallengeNotice:noticeData inView:topView];
		}
	}
	else
	{
...
- (void)showDefaultNotice
{
	if(defaultBackgroundNotice)
	{
		[self showBackgroundNotice:defaultBackgroundNotice andStatus:defaultStatus];
	}
