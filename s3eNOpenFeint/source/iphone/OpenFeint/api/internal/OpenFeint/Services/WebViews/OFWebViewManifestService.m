...
static OFWebViewCacheLoader* loader;
static NSString *rootPath;
static NSString *manifestApplication;
...
+(NSArray*) sha1Errors {
    ...
    for(NSString* path in localManifest) {
        ...
        OFSHA1Init(&ctx);
        OFSHA1Update(&ctx, (unsigned char*) fileData.bytes, fileData.length);
        OFSHA1Final(digest, &ctx);
        [fileData release];
