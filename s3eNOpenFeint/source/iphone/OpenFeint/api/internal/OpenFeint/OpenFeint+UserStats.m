...
static NSTimeInterval inactiveTimeThisSession = 0;
...
+ (void)sessionActive
{
	if (sessionActiveAt == 0) 
	{
        ...
        if (sessionNotActiveAt != 0)
        {
            inactiveTimeThisSession += sessionActiveAt - sessionNotActiveAt;
        }
...
+ (void) getUserStatsParams:(OFQueryStringWriter*)params
{
	[params ioNSStringToKey:@"[user_stats]total_dashboard_duration"  object:( [NSString stringWithFormat:@"%d", [OpenFeint totalDashboardDuration]])];
	[params ioNSStringToKey:@"[user_stats]total_dashboard_launches"  object:( [NSString stringWithFormat:@"%d", [OpenFeint numberOfDashboardLaunches]])];
	[params ioNSStringToKey:@"[user_stats]total_game_session_duration"  object:( [NSString stringWithFormat:@"%d", [OpenFeint totalGameSessionsDuration]])];
	[params ioNSStringToKey:@"[user_stats]total_game_sessions"  object:( [NSString stringWithFormat:@"%d", [OpenFeint numberOfGameSessions]])];
	[params ioNSStringToKey:@"[user_stats]total_online_game_sessions"  object:( [NSString stringWithFormat:@"%d", [OpenFeint numberOfOnlineGameSessions]])];
	[params ioNSStringToKey:@"[user_stats]version" object:@"2"];
}
...
+ (NSTimeInterval)sessionPlayTime
{
    if (sessionNotActiveAt != 0)
    {
        // The app is not active.  Get the time from session start time to when it went inactive.
        NSDate* inactiveDate = [NSDate dateWithTimeIntervalSince1970:sessionNotActiveAt];
        return [inactiveDate timeIntervalSinceDate:[OFSessionInfo sharedInstance].sessionStartDate] - inactiveTimeThisSession;
    }
    return [[NSDate date] timeIntervalSinceDate:[OFSessionInfo sharedInstance].sessionStartDate] - inactiveTimeThisSession;
}
