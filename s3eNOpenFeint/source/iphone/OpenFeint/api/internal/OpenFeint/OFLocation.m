...
- (void) dealloc 
{
	[self stopUpdatingLocation]; //Just incase...
...
//		OFDelegate locationFailed(self, @selector(_onSetLocationFailed:));
		[OFUserService setUserLocation:nil location:userLocation allowed:userAllowed 
                             onSuccessInvocation:nil 
                             onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_onSetLocationFailed:)]];
	}
	else if (tries++ < 10)
	{
