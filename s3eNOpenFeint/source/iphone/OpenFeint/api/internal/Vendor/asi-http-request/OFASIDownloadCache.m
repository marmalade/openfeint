...
#import "OFASIDownloadCache.h"
#import "OFASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static OFASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";

@interface OFASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
@end

@implementation OFASIDownloadCache
...
+ (id)sharedCache
{
	if (!sharedCache) {
		...
		[sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"OFASIHTTPRequestCache"]];
...
- (void)storeResponseForRequest:(OFASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	[[self accessLock] lock];
	...
	[responseHeaders setObject:[[[self class] rfc1123DateFormatter] stringFromDate:[NSDate date]] forKey:@"X-OFASIHTTPRequest-Fetch-date"];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(OFASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (NSString *)pathToStoreCachedResponseHeadersForRequest:(OFASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (void)removeCachedDataForRequest:(OFASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (BOOL)isCachedDataCurrentForRequest:(OFASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	if ([self shouldRespectCacheControlHeaders]) {
		...
		if (cacheControl) {
			...
			if ([scanner scanString:@"max-age" intoString:NULL]) {
				...
				NSDate *fetchDate = [OFASIHTTPRequest dateFromRFC1123String:[cachedHeaders objectForKey:@"X-OFASIHTTPRequest-Fetch-date"]];
		...
		if (expires) {
			if ([[OFASIHTTPRequest dateFromRFC1123String:expires] timeIntervalSinceNow] >= 0) {
				[[self accessLock] unlock];
				return YES;
			}
		}
...
+ (BOOL)serverAllowsResponseCachingForRequest:(OFASIHTTPRequest *)request
{
...
+ (NSString *)keyForURL:(NSURL *)url
{
	...
	// Strip trailing slashes so http://allseeing-i.com/OFASIHTTPRequest/ is cached the same as http://allseeing-i.com/OFASIHTTPRequest
...
+ (NSDateFormatter *)rfc1123DateFormatter
{
	...
	NSDateFormatter *dateFormatter = [threadDict objectForKey:@"OFASIDownloadCacheDateFormatter"];
	if (dateFormatter == nil) {
		dateFormatter = [[[NSDateFormatter alloc] init] autorelease];
		[dateFormatter setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"] autorelease]];
		[dateFormatter setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
		[dateFormatter setDateFormat:@"EEE, dd MMM yyyy HH:mm:ss 'GMT'"];
		[threadDict setObject:dateFormatter forKey:@"OFASIDownloadCacheDateFormatter"];
	}
	return dateFormatter;
}


- (BOOL)canUseCachedDataForRequest:(OFASIHTTPRequest *)request
{
