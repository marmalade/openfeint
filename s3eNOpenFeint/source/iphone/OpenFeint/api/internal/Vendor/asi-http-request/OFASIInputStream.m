...
+ (id)inputStreamWithFileAtPath:(NSString *)path request:(OFASIHTTPRequest *)theRequest
{
	OFASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(OFASIHTTPRequest *)theRequest
{
	OFASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask OFASIHTTPRequest for the maximum amount of data we can read
