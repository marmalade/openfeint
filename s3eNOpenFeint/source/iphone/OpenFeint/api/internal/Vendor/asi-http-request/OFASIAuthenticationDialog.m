...
@implementation OFASIAutorotatingViewController
...
+ (void)presentAuthenticationDialogForRequest:(OFASIHTTPRequest *)theRequest
{
...
- (void)orientationChanged:(NSNotification *)notification
{
    ...
    //OF: fixed this from ui orientation to device orientation (fix warnings).  We aren't using this anyway.
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[OFASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (OFASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (OFASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		OFASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
