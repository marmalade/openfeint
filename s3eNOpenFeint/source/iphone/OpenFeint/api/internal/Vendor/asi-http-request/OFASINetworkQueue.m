...
- (void)resetProgressDelegate:(id *)progressDelegate
{
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		double max = 1.0;
		[OFASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&max callerToRetain:nil];
	}
	selector = @selector(setDoubleValue:);
	if ([*progressDelegate respondsToSelector:selector]) {
		double value = 0.0;
		[OFASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#else
...
// Only add OFASIHTTPRequests to this queue!!
...
- (void)request:(OFASIHTTPRequest *)request didReceiveBytes:(long long)bytes
{
	...
	if ([self downloadProgressDelegate]) {
		[OFASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}
...
- (void)request:(OFASIHTTPRequest *)request didSendBytes:(long long)bytes
{
	...
	if ([self uploadProgressDelegate]) {
		[OFASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}
