...
@class OFASIDataDecompressor;

extern NSString *OFASIHTTPRequestVersion;
...
// The error domain that all errors generated by OFASIHTTPRequest use
extern NSString* const OFNetworkRequestErrorDomain;
...
extern unsigned long const OFASIWWANBandwidthThrottleAmount;
...
@interface OFASIHTTPRequest : NSOperation <NSCopying> {
	...
	id <OFASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but OFASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <OFASIHTTPRequestDelegate, OFASIProgressDelegate> queue;
	...
	// Automatically set to true in OFASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when OFASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	id <OFASIProgressDelegate> uploadProgressDelegate;
	...
	id <OFASIProgressDelegate> downloadProgressDelegate;
	...
	// When YES, OFASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, OFASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - OFASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	OFASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in OFASINetworkQueue.h
	...
	// Tells OFASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let OFASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// OFASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// When YES, OFASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Will be OFASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	...
	// The download cache that will be used for this request (use [OFASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <OFASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See OFASICacheDelegate.h for possible values
	ASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See OFASICacheDelegate.h for possible values
	...
	OFASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using OFASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	OFASIHTTPRequest *PACFileRequest;
...
// On iPhone, OFASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) OFASIHTTPRequest *mainRequest;
...
@property (assign) id <OFASICacheDelegate> downloadCache;
...
#endif
@property (retain) OFASIDataDecompressor *dataDecompressor;
