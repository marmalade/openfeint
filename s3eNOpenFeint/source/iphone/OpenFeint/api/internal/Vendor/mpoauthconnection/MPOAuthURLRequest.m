...
static NSString* gBoundaryString = @"sadf8as9fha3fpaef8ah";
...
- (id)initWithURL:(NSURL *)inURL andParameters:(NSArray *)inParameters {
	...
	if (self != nil) {
		...
		BOOL parentalControlsEnabled = [OpenFeint session].currentDevice.parentalControls.enabled;
				
		addParameter(_parameters, @"info_client_application_version",	[[OFSettings instance] clientBundleVersion]);
		addParameter(_parameters, @"info_client_application_bundle_id",	[[OFSettings instance] clientBundleIdentifier]);
		addParameter(_parameters, @"info_client_openfeint_version",		[NSString stringWithFormat:@"%d", [OpenFeint versionNumber]]);
		addParameter(_parameters, @"info_client_device_locale",			[[OFSettings instance] clientLocale]);
		addParameter(_parameters, @"session_id",						[OFSessionInfo sharedInstance].sessionId);
		addParameter(_parameters, @"parental_controls_enabled",			parentalControlsEnabled ? @"true" : @"false");
...
- (NSData*)createMultipartFormContent:(NSArray*)parameters
{
	...
	NSData* boundaryMarker = [[self getBoundaryMarkerStart] dataUsingEncoding:NSUTF8StringEncoding];
	NSString* headerSpacer = @"\r\n";
	...
	for (MPURLRequestParameter* param in parameters)
	{
		...
		if (param.value)
		{
			NSString* headerContentDisposition = [NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n", param.name];
		...
		{
			NSString* headerContentDisposition = [NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"; filename=\"%@.%@\"\r\n", param.name, param.name, param.blobDataType];
			NSString* headerContentType = [NSString stringWithFormat:@"Content-Type: %@\r\n", param.blobDataType];
			NSString* headerContentEncoding =  [NSString stringWithFormat:@"Content-Transfer-Encoding: binary\r\n"];	
		...
		{
			OFAssert(NO, @"Parameter is neither ASCII nor binary");
		}
	}
	
	NSString* boundaryEndMarker = [self getBoundaryMarkerEnd];;
...
- (NSURLRequest  *)urlRequestSignedWithSecret:(NSString *)inSecret usingMethod:(NSString *)inScheme withExtraParameters:(NSArray*)extraParameters
{
	...
	else if ([[self HTTPMethod] isEqualToString:@"POST"]) 
	{
		...
		static BOOL sDebugPrint = NO;
		if(sDebugPrint)
		{
			OFLog(@"\n------------------------------------------------------------------------------------------\n");
			OFLog(@"Request Body: %@", [[[NSString alloc] initWithData:aRequest.HTTPBody encoding:NSUTF8StringEncoding] autorelease]);
			OFLog(@"\n------------------------------------------------------------------------------------------\n");
		}
		
	}
	
//	OFLog(@"Request: %@", aRequest);
