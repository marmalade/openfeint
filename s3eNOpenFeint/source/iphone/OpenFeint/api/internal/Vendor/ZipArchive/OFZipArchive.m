...
@interface OFZipArchive (Private)
...
{
	_zipFile = of_zipOpen( (const char*)[zipFile UTF8String], 0 );
	if( !_zipFile ) 
		return NO;
	return YES;
}
...
//	zipInfo.dosDate = (unsigned long) current;
	...
	{
		ret = of_zipOpenNewFileInZip( _zipFile,
	...
	else
	{
		...
		ret = of_zipOpenNewFileInZip3( _zipFile,
								  (const char*) [newname UTF8String],
								  &zipInfo,
								  NULL,0,
								  NULL,0,
	...
	ret = of_zipWriteInFileInZip( _zipFile, (const void*)[data bytes], dataLen);
	if( ret!=Z_OK )
	{
		return NO;
	}
	ret = of_zipCloseFileInZip( _zipFile );
	if( ret!=Z_OK )
		return NO;
	return YES;
}
...
-(BOOL) CloseZipFile2
{
	...
	if( _zipFile==NULL )
		return NO;
	BOOL ret =  of_zipClose( _zipFile,NULL )==Z_OK?YES:NO;
	_zipFile = NULL;
	return ret;
}
...
{
	_unzFile = of_unzOpen( (const char*)[zipFile UTF8String] );
	if( _unzFile )
	{
		unz_global_info  globalInfo = {0};
		if( of_unzGetGlobalInfo(_unzFile, &globalInfo )==UNZ_OK )
		{
...
{
	BOOL success = YES;
	int ret = of_unzGoToFirstFile( _unzFile );
	...
	do{
		if( [_password length]==0 )
			ret = of_unzOpenCurrentFile( _unzFile );
		else
			ret = of_unzOpenCurrentFilePassword( _unzFile, [_password cStringUsingEncoding:NSASCIIStringEncoding] );
		if( ret!=UNZ_OK )
		{
		...
		ret = of_unzGetCurrentFileInfo(_unzFile, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
		if( ret!=UNZ_OK )
		{
			[self OutputErrorMessage:@"Error occurs while getting file info"];
			success = NO;
			of_unzCloseCurrentFile( _unzFile );
			break;
		}
		char* filename = (char*) malloc( fileInfo.size_filename +1 );
		of_unzGetCurrentFileInfo(_unzFile, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
		...
		if( [fman fileExistsAtPath:fullPath] && !isDirectory && !overwrite )
		{
			...
			{
				of_unzCloseCurrentFile( _unzFile );
				ret = of_unzGoToNextFile( _unzFile );
				continue;
			}
		}
		...
		while( fp )
		{
			read=of_unzReadCurrentFile(_unzFile, buffer, 4096);
			if( read > 0 )
			{
...
-(BOOL) UnzipCloseFile
{
	_password = nil;
	if( _unzFile )
		return of_unzClose( _unzFile )==UNZ_OK;
	return YES;
}
