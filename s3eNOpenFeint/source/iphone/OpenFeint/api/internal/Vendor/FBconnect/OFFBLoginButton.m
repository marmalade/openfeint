...
@implementation OFFBLoginButton
...
+ (void)initialize {
	if (self == [OFFBLoginButton class]) {
#ifdef __IPHONE_3_0
		// Try to load the accessibility trait values on OS 3.0
		traitImage = (UIAccessibilityTraits*) dlsym(RTLD_SELF, "UIAccessibilityTraitImage");
		traitButton = (UIAccessibilityTraits*) dlsym(RTLD_SELF, "UIAccessibilityTraitButton");
#endif
	}
}
...
- (UIImage*)buttonImage {
  if (_session.isConnected) {
    return [UIImage imageFromFBBundleNamed:@"images/logout.png"];
  } else {
    if (_style == FBLoginButtonStyleNormal) {
      return [UIImage imageFromFBBundleNamed:@"images/login.png"];
    } else if (_style == FBLoginButtonStyleWide) {
      return [UIImage imageFromFBBundleNamed:@"images/login2.png"];
    } else {
      return nil;
    }
  }
...
- (UIImage*)buttonHighlightedImage {
  if (_session.isConnected) {
    return [UIImage imageFromFBBundleNamed:@"images/logout_down.png"];
  } else {
    if (_style == FBLoginButtonStyleNormal) {
      return [UIImage imageFromFBBundleNamed:@"images/login_down.png"];
    } else if (_style == FBLoginButtonStyleWide) {
      return [UIImage imageFromFBBundleNamed:@"images/login2_down.png"];
    } else {
      return nil;
    }
  }
...
- (void)touchUpInside {
  if (_session.isConnected) {
    ...
    OFFBLoginDialog* dialog = [[[OFFBLoginDialog alloc] initWithSession:_session] autorelease];
    [dialog show];
  }
}
...
- (void)initButton {
  ...
  self.session = [OFFBSession session];
}
...
- (void)sessionDidLogout:(OFFBSession*)session {
  [self updateImage];
}
