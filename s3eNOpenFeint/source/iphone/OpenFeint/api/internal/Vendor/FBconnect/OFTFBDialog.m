...
@implementation UIImage(OpenFeintFBConnect)

+ (id)imageFromFBBundleNamed:(NSString*)imageName
{
	NSString * FBBundlePath = [[OpenFeint getResourceBundle] pathForResource:@"FBConnect" ofType:@"bundle"];
	FBBundlePath = [FBBundlePath stringByAppendingString:@"/"];
	NSString * imagePath = [FBBundlePath stringByAppendingString:imageName];
...
- (id)init {
  return [self initWithSession:[OFFBSession session]];
}
...
- (id)initWithSession:(OFFBSession*)session {
  ...
  if (self != nil) {
	...
	UIImage* iconImage = [UIImage imageFromFBBundleNamed:@"images/fbicon.png"];
    UIImage* closeImage = [UIImage imageFromFBBundleNamed:@"images/close.png"];
    ...
    CGFloat titleLabelFontSize = (OFFBIsDeviceIPad() ? 18 : 14);
