...
@implementation OFFBNativeStringUploader
...
+ (int) uploadStringSet:(NSDictionary *)dictionary
  ...
  OFFBUploadResultReceiver *requestResultReceiver = 
    [[[OFFBUploadResultReceiver alloc] init] autorelease];

  [requestResultReceiver requestWithParams:
    [NSDictionary dictionaryWithObject:query forKey:@"native_strings"]];

  if (error) *error = requestResultReceiver._uploadError;
