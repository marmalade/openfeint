...
NSObject<OFFBTranslationsLoaderDelegate> *_loadDelegate = nil;
OFFBTranslationsLoader* _sharedInstance = nil;
...
@implementation OFFBTranslationsLoader

+ (OFFBTranslationsLoader*) sharedInstance {
  @synchronized(self) {
    if (nil == _sharedInstance)
      _sharedInstance = [[OFFBTranslationsLoader alloc] init];
  }
...
- (void) executeRequest:(NSString *)locale {
  ...
  NSDictionary *params = [NSDictionary dictionaryWithObject:locale 
                                                     forKey:@"locale"];
  [[OFFBRequest requestWithDelegate:self] call:@"facebook.intl.getTranslations" 
                                      params:params];
}
...
- (void) loadTranslationsForLocale:(NSString *)locale
                          delegate:(NSObject<OFFBTranslationsLoaderDelegate> *)
                            delegate {
  if (nil == locale ||
      [locale isEqualToString:@""] ||
       ![OFFBTranslationsLoader getFbtLocaleFor:locale]) {
    [NSException raise:@"NoLocaleException"
                format:@"Locale was empty string, nil, not recognized,\
 or could not be detected."];
  }
  locale = [OFFBTranslationsLoader getFbtLocaleFor:locale];
...
+ (void) loadTranslationsForLocale:(NSString *)locale 
                          delegate:(NSObject<OFFBTranslationsLoaderDelegate> *)
                            delegate {
  [[OFFBTranslationsLoader sharedInstance] loadTranslationsForLocale:locale
                                                          delegate:delegate];
}
...
- (void) loadTranslations:(NSObject<OFFBTranslationsLoaderDelegate> *)delegate {
  _loadDelegate = delegate;
 ...
 */
+ (void) loadTranslations:(NSObject<OFFBTranslationsLoaderDelegate> *)delegate {
  [[OFFBTranslationsLoader sharedInstance] loadTranslations:delegate];
}
...
- (void) request:(OFFBRequest *)request didLoad:(id)result {
...
- (void) request:(OFFBRequest *)request didFailWithError:(NSError *)error {
...
NSString* FBLocalizedString(NSString *key, NSString *comment) {
  return [[OFFBTranslationsLoader sharedInstance] stringWithString:key
                                                         comment:comment];
}
