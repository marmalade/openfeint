...
#import "OFFBConnectGlobal.h"

@protocol OFFBRequestDelegate;
@class OFFBSession;

@interface OFFBRequest : NSObject {
 ...
 */
+ (OFFBRequest*)requestWithDelegate:(id<OFFBRequestDelegate>)delegate;

/**
 * Creates a new API request for a particular session.
 */
+ (OFFBRequest*)requestWithSession:(OFFBSession*)session;

/**
 * Creates a new API request for the global session with a delegate.
 */
+ (OFFBRequest*)requestWithSession:(OFFBSession*)session delegate:(id<OFFBRequestDelegate>)delegate;

@property(nonatomic,assign) id<OFFBRequestDelegate> delegate;
 ...
 */
- (id)initWithSession:(OFFBSession*)session;
 ...
 */
- (void)requestLoading:(OFFBRequest*)request;

/**
 * Called when the server responds and begins to send back data.
 */
- (void)request:(OFFBRequest*)request didReceiveResponse:(NSURLResponse*)response;
 ...
 */
- (void)requestWasCancelled:(OFFBRequest*)request;
