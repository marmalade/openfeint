...
+ (OFFBRequest*)request {
  return [self requestWithSession:[OFFBSession session]];
}

+ (OFFBRequest*)requestWithDelegate:(id<OFFBRequestDelegate>)delegate {
  return [self requestWithSession:[OFFBSession session] delegate:delegate];
}

+ (OFFBRequest*)requestWithSession:(OFFBSession*)session {
  return [[[OFFBRequest alloc] initWithSession:session] autorelease];
}

+ (OFFBRequest*)requestWithSession:(OFFBSession*)session delegate:(id<OFFBRequestDelegate>)delegate {
  OFFBRequest* request = [[[OFFBRequest alloc] initWithSession:session] autorelease];
...
- (id)parseXMLResponse:(NSData*)data error:(NSError**)error {
  OFFBXMLHandler* handler = [[[OFFBXMLHandler alloc] init] autorelease];
