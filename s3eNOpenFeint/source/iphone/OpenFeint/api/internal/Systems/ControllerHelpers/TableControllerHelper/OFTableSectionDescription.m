...
-(OFTableSectionRowData)gatherDataForRow:(NSUInteger) row
{
    OFTableSectionRowData rowData = {0};
    [self matchupExpandedViews];
    if(self.leadingCellName) {
        if(row == 0) {
            rowData.cellType = OFTableSectionCellTypeLeading;
            return rowData;
        }
        --row;
    }
    ...
    if(row < [self.staticCells count]) {
        rowData.dataRow = row;
        rowData.cellType = OFTableSectionCellTypeStatic;
        return rowData;
    }
    rowData.dataRow = 0;
    BOOL isActionCell = NO;
    while(row && rowData.dataRow < [self.page count]) {
        if(row == 0) break;
        --row;
        if([[self.expandedViews objectAtIndex:rowData.dataRow] boolValue]) {
            if(row == 0) {
                isActionCell = YES;
                break;
            }
    ...
    if(rowData.dataRow == [self.page count]) {
        rowData.cellType = (self.trailingCellName && row==0) ? OFTableSectionCellTypeTrailing : OFTableSectionCellTypeError;
    }
    else {
        rowData.cellType = isActionCell ? OFTableSectionCellTypeAction : OFTableSectionCellTypeResource;
    }
    return rowData;
}
...
-(void)runTestLength:(NSUInteger) len {
    for(unsigned int i=0; i<len; ++i) {
        OFTableSectionRowData data = [self gatherDataForRow:i];
        OFLog(@"Row %d is type %d  dataRow %d", i, data.cellType, data.dataRow);
    }
}
