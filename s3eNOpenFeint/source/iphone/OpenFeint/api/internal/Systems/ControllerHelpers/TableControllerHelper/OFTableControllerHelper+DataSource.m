...
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return !([self isCellLeadingOrTrailingAtPath:indexPath]);
}
...
- (void)_confirmDelete
{
	if (deletingIndexPath)
	{
		...
		OFResource* cellResource = [[self getCellAtIndexPath:deletingIndexPath] retain];
        OFTableSectionRowData data = [section gatherDataForRow:deletingIndexPath.row];
		[section.page.objects removeObjectAtIndex:data.dataRow];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (shouldConfirm)
	{
		OFResource* cellResource = [self getCellAtIndexPath:deletingIndexPath];
		...
		// showInView: self.view];   //this will cause the bottom of cancel button to be hided from clicking, and also has the wrong orientation problem 
