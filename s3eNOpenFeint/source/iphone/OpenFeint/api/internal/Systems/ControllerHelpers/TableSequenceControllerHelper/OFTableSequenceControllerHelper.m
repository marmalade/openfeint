...
@interface OFTableSequenceControllerHelper()
@property (nonatomic,retain) NSMutableDictionary* resourceControllerMap;
@end


@implementation OFTableSequenceControllerHelper
@synthesize resourceControllerMap = mResourceControllerMap;
- (void)dealloc
{
	OFSafeRelease(mMetaDataObjects);
    self.resourceControllerMap = nil;
	[super dealloc];
}
...
- (void)viewDidLoad
{
	...
	mIsLoadingNextPage = NO;
	mNumLoadedPages = 0;
    self.resourceControllerMap = [NSMutableDictionary dictionaryWithCapacity:10];
...
- (void)_onDataLoaded:(OFPaginatedSeries*)resources isIncremental:(BOOL)isIncremental
{
	...
	if(!isFirstTimeShowingData)
	{
		...
		if(!allowStreamingPagination && isIncremental)
		{
			...
			const BOOL shouldPrependContents = ([self _isDataPaginated] == NO);
