...
- (NSString*) getCellControllerNameFromSection:(NSArray*)sectionCells atRow:(NSUInteger)row
{
    ...
    return [self.resourceControllerMap objectForKey:[resource class]];
}
...
- (OFResource*)getResourceAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    OFTableSectionRowData data = [curSection gatherDataForRow:[indexPath indexAtPosition:1]];
	return [curSection.page objectAtIndex:data.dataRow];
}
