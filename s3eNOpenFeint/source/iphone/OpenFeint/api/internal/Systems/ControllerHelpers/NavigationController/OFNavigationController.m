...
@synthesize loadingViewParent = mLoadingViewParent;
...
{
	OFTabBarContainer* tabBarContainer = (OFTabBarContainer*)[self.view findSuperviewByClass:[OFTabBarContainer class]];
	OFTabBar* tabBar = nil;
	...
	if (tabBar)
	{
		...
		for (UIView* view in tabBarContainer.subviews)
		{
			...
			if ([view isKindOfClass:[OFTabBar class]])
			{
                ...
                {
                    dist = 376.0f + tabBarFrame.size.height;
                }
                else
                    dist = tabBarFrame.size.height;
                dist += [OFBadgeView minimumSize].height;
...
- (void)addBackgroundAndOverlayImage:(UIImage*)bgImage
{
...
- (void)viewDidLoad
{
    ...
    if (!self.loadingViewParent) self.loadingViewParent = self.view;
    
    UIImage *bgImage = nil;
    bgImage = [[OFImageLoader loadImage:@"OFNavBar.png"] stretchableImageWithLeftCapWidth:5 topCapHeight:0];
	[self addBackgroundAndOverlayImage:bgImage];
}
...
- (void)dealloc
{
    ...
    OFSafeRelease(mLoadingViewParent);
	[super dealloc];
}
...
- (void)showLoadingIndicator
{
	...
	else
	{
		...
		OFIntroNavigationController* introNav = [OFIntroNavigationController activeIntroNavigationController];
        if (self == [introNav navController])
        {
			[introNav displayLoadingView:mLoadingController.view];
        }
        else
        {
            mLoadingController.view.frame = self.view.bounds;
			[mLoadingViewParent insertSubview:mLoadingController.view belowSubview:self.navigationBar];
        }
