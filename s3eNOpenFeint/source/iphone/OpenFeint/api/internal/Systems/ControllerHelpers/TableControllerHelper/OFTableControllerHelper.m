...
- (OFInvocation*)onSuccessInvocation;
{
    return [OFInvocation invocationForTarget:self selector:@selector(_onDataLoadedWrapper:)];
//	return OFDelegate(self, @selector(_onDataLoadedWrapper:));
}

- (OFInvocation*)onFailureInvocation;
{
    return [OFInvocation invocationForTarget:self selector:@selector(_onDataFailedLoading)];
...
- (BOOL)isInHiddenTab
{
	...
	OFAssert(!navController || [navController isKindOfClass:[OFNavigationController class]], @"only use OFNavigationControllers");
...
- (void)viewDidLoad
{
    ...
    mHelperCellsForHeight = [[NSMutableDictionary alloc] initWithCapacity:1];
...
- (void)_displayEmptyDataSetView:(UIViewController*)optionalViewController andMessage:(NSString*)message
{
	...
	{
		mEmptyTableController = (OFDeadEndErrorController*)[[[OFControllerLoaderObjC loader] load:@"EmptyDataSet"] retain]; //load(@"EmptyDataSet") retain];
...
{	
	...
	if(tableHeaderControllerName)
	{
		if(!mTableHeaderController)
		{
			...
			mTableHeaderController = [[[OFControllerLoaderObjC loader] load:tableHeaderControllerName owner:self] retain];// load(tableHeaderControllerName, self) retain];
	...
	else if (tableHeaderViewName)
	{
		if (!mTableHeaderView)
		{
			...
			mTableHeaderView = [[[OFControllerLoaderObjC loader] loadView:tableHeaderViewName owner:self] retain];// loadView(tableHeaderViewName, self) retain];
...
- (OFResource*)getCellAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    NSUInteger resourceIndex = [self hasStreamingSections] ? data.dataRow - [self _getLoadedBaseRowIndex] : data.dataRow;
...
- (void)toggleExpansionAtIndexPath:(NSIndexPath*)indexPath {
    OFTableSectionDescription* section = [self getSectionForIndexPath:indexPath];
    OFTableSectionRowData data = [section gatherDataForRow:indexPath.row];
    NSAssert(data.cellType == OFTableSectionCellTypeResource, @"Trying to expand a non-resource cell");
    BOOL wasExpanded = [[section.expandedViews objectAtIndex:data.dataRow] boolValue];
    [section.expandedViews replaceObjectAtIndex:data.dataRow withObject:[NSNumber numberWithBool:!wasExpanded]];
...
-(BOOL)isExpandedAtIndexPath:(NSIndexPath*)indexPath {
    OFTableSectionDescription* section = [self getSectionForIndexPath:indexPath];
    OFTableSectionRowData data = [section gatherDataForRow:indexPath.row];
    if(data.cellType == OFTableSectionCellTypeResource) {
        return [[section.expandedViews objectAtIndex:data.dataRow] boolValue];
    }
    else {
        return NO;
    }
