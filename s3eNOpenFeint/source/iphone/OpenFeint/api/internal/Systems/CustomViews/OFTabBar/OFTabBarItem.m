...
- (void)setActive:(BOOL)activeValue
{
	if( !disabled )
	{
		...
		if (active)
		{
			...
			{
				BOOL justCreatedViewController = NO;
				if (self.viewController == nil)
				{
					...
					justCreatedViewController = YES;
				}
				
				//Only call this if we are switching to the tab, or we just created the controller even if we aren't switching active states (later case is during the "try OpenFeint" flow after you logout, and try to launch the dashboard).
				if(!wasActive || justCreatedViewController)
				{
...
{
	[badgeView setValueText:value];
}

-(void) refreshBadge {
    NSString* badgeValue = [self.badgeSelector length] ? [OpenFeint performSelector:sel_registerName([self.badgeSelector UTF8String])] : @"0";
    [badgeView setValueText:badgeValue];
}


-(void) setOnlineStatus:(BOOL) onlineStatus {
    if([self.viewController respondsToSelector:@selector(setOnlineStatus:)]) {
        [(id)self.viewController setOnlineStatus:onlineStatus];
    }
}
