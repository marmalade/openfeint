...
UIImage* getImage(NSString* imagePath) {
    if([imagePath hasPrefix:@"cache:"]) {        
        return [UIImage imageWithContentsOfFile:[[OFWebViewManifestService rootPath] stringByAppendingPathComponent:[imagePath substringFromIndex:6]]];
    }
    else {
        return imagePath ? [OFImageLoader loadImage:imagePath] : nil;
    }
}
...
{
    UIImage* activeImage = getImage(activeImagePath);
    UIImage* inactiveImage = getImage(inactiveImagePath);
    UIImage* disabledImage = getImage(disabledImagePath);
...
- (void)addViewController:(NSString*)controllerName 
 ofDisabledControllerName:(NSString*)ofDisabledControllerName 
			...
			inactiveImage:(NSString*)inactiveImagePath 
			disabledImage:(NSString*)disabledImagePath 
			andBadgeSelector:(NSString*)badgeSelector
{
    UIImage* activeImage = getImage(activeImagePath);
    UIImage* inactiveImage = getImage(inactiveImagePath);
    UIImage* disabledImage = getImage(disabledImagePath);
    ...
    [[mTabBar.items lastObject] setBadgeSelector:badgeSelector];
}
