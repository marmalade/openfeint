...
- (void)awakeFromNib
{
	...
	UIFont* defaultFont = [OpenFeint isLargeScreen] ? [UIFont fontWithName:@"Helvetica" size:13.f] : [UIFont fontWithName:@"Helvetica" size:12.f];

	NSString* testString = @"ABCDEFGHIJKLMNOPQRSTUVWXYZ";
	lineHeight = [testString sizeWithFont:defaultFont constrainedToSize:CGSizeMake(FLT_MAX, 0)].height;
	...
	internalTextField = [[UITextField alloc] initWithFrame:textViewFrame];
	internalTextField.userInteractionEnabled = NO;
	internalTextField.borderStyle = UITextBorderStyleRoundedRect;
	internalTextField.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
	...
	internalTextView.font = defaultFont;
