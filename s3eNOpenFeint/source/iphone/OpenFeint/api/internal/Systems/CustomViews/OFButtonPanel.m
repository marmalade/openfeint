...
@synthesize centeredVertically;
@synthesize fixedNumColumns;
@synthesize withNavBar;
...
- (void)layoutSubviews
{
	...
	float cols = (float)fixedNumColumns;
	if (fixedNumColumns == 0)
		cols = floorf((self.frame.size.width + buttonSpacing.width) / (buttonSize.width + buttonSpacing.width));
	...
	initialPos.y = centeredVertically ? roundf(initialPos.y) : 0.f;
	...
	{
        if (y == 0 && maxButtonCount > 3 && withNavBar) pos.y -= 15.f;
	...
	while (button != nil)
	{
		...
		{
            // Takeshi note: just comment it out for now
