...
{
    // don't draw where the content frame border covers up.
    rect = contentFrameView && !contentFrameView.hidden? CGRectInset(rect, 8, 8) : rect;
	...
	if (mPattern != nil)
	{
        [mPattern drawAsPatternInRectNoMemLeak:rect];
	}
...
- (id)initWithCoder:(NSCoder*)aDecoder
{
	...
	if (self != nil)
	{
        ...
        if ([OpenFeint isLargeScreen]) self.backgroundColor = [UIColor clearColor];
...
- (id)initWithCoder:(NSCoder*)aDecoder
{
	...
	if (self != nil)
	{
        ...
        contentFrameView = [[[OFContentFrameView alloc] initWithFrame:contentFrameRect] autorelease];
        contentFrameView.tag = 101;
        contentFrameView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        [self addSubview:contentFrameView];
	}
	return self;
}
