...
- (BOOL)tryGet:(NSString*)dottedProperty outObject:(id*)outObject
{
	...
	{
		SEL discoveredGetter = [returnedObject findSelectorForPropertyNamed:[propertyNames objectAtIndex:i] forReading:YES];
		if(!discoveredGetter)
		{
			return NO;
		}
...
- (BOOL)trySet:(NSString*)dottedProperty withValue:(id)value
{
	...
	SEL discoveredSetter = [objectToSet findSelectorForPropertyNamed:[propertyNames lastObject] forReading:NO];
	if(!discoveredSetter)
	{
		return NO;
	}
