...
#pragma mark OFASIHTTPRequest extensions

// In lieu of modules, we are just going to derive from OFASIHTTPRequest and ASIFormDataRequest, overriding the
...
@protocol XPHelper <NSObject>
- (void)setXPRequest:(OFXPRequest*)_request;
- (void)requestFinished;
@end

@interface ExtendedGetRequest : OFASIHTTPRequest <ExtendedRequest, XPHelper>
{
}
@property (nonatomic, retain)OFXPRequest* xpRequest;
@end

@implementation ExtendedGetRequest
@synthesize xpRequest;
- (void)dealloc
{
    self.xpRequest = nil;
    [super dealloc];
...
@interface ExtendedFormRequest : OFASIFormDataRequest <ExtendedRequest, XPHelper>
{
}
@property (nonatomic, retain)OFXPRequest* xpRequest;
@end
...
- (void)addArgument:(id)arg forKey:(NSString*)key
{
	...
	{
		accum = [[NSMutableString stringWithFormat:@"%@=%@", uriEncode(key), uriEncode(arg)] retain];
	}
}
- (NSString*)queryString
{
	return [[accum copy] autorelease];
}	
@end
...
- (void)setRequest:(OFASIHTTPRequest*)_request
{
    if (_request == mRequest) return;
    
    [mRequest release];
    mRequest = [_request retain];
    
    if (_request)
    {
        [(id<XPHelper>)_request setXPRequest:self];
    }
    else
    {
        [(id<XPHelper>)_request setXPRequest:nil];
    }
}
...
- (void)genRequestWithURL:(NSURL*)url andASIClass:(Class)OFASIHTTPRequestSubclass
{
    OFASIHTTPRequest* req = [[OFASIHTTPRequestSubclass alloc] initWithURL:url];
...
- (void)genRequestWithPath:(NSString*)path andASIClass:(Class)OFASIHTTPRequestSubclass
{
    static NSString* sServerURL = nil;
    if(!sServerURL)
    {
        sServerURL = [[[[NSURL URLWithString:[[OFSettings instance] getSetting:@"server-url"]] standardizedURL] absoluteString] retain];
    }
	...
	[self genRequestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", sServerURL, path]] andASIClass:(Class)OFASIHTTPRequestSubclass];
}
...
+ (id)requestWithPath:(NSString*)_path andASIClass:(Class)OFASIHTTPRequestSubclass
{
	OFXPRequest* req = [self request];
    req.path = _path;
    [req genRequestWithPath:_path andASIClass:OFASIHTTPRequestSubclass];
	return req;
}
...
+ (id)putRequestWithPath:(NSString*)path andBodyString:(NSString*)bodyString
{
    ...
    ((OFASIHTTPRequest*)req.request).postBody = [NSMutableData dataWithData:[bodyString dataUsingEncoding:NSUTF8StringEncoding]];
	return req;
}
