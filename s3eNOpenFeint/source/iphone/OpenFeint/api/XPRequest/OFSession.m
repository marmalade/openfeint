    ...
    OFResourceRequest* request = [OFResourceRequest 
        ...
        andBody:[NSDictionary dictionaryWithObjectsAndKeys:
                 ...
                 [NSDictionary dictionaryWithObjectsAndKeys:
                  ...
                  @"dunno", @"processor",
                  nil], @"device", 
                 [OpenFeint gameSpecificDeviceIdentifier], @"gsdi",
                 [OFSession protocolVersion], @"protocol_version",
                 [[OFSettings instance] clientBundleVersion], @"game_version",
                 nil]];
...
- (void)loginNewUser
{
	...
	stateFlags.sessionChangeInProgress = YES;
    stateFlags.bootstrappingDeferred = NO;

    [OFBootstrapService
        doBootstrapWithNewAccount:YES
        userId:nil
     onSucceededLoggingIn:[OFInvocation invocationForTarget:self selector:@selector(bootstrapSuccess:)]
     onFailedLoggingIn:[OFInvocation invocationForTarget:self selector:@selector(bootstrapFailure:)]];
}
...
- (void)loginUserId:(NSString*)userId password:(NSString*)password
{
    ...
    OFUser* user = [OpenFeint localUser];
    // we need to bootstrap if given userId is 0 or nil, and also if local user is different from userId (switch account)
    if ([userId isEqualToString:@"0"] || [userId length] == 0 || ![userId isEqualToString:user.userId])
    {
        stateFlags.bootstrappingDeferred = NO;
    ...
    {
        stateFlags.bootstrappingDeferred = YES;
        [self establishUserSessionForId:user.userId password:nil];
    }
}
...
- (void)establishUserSessionForId:(NSString*)userId password:(NSString*)password
{
	NSMutableDictionary* body = [NSMutableDictionary dictionaryWithObjectsAndKeys:
        ...
        [[OFSettings instance] clientBundleVersion], @"app-version",
        nil];
...
{
    stateFlags.sessionChangeInProgress = NO;
    BOOL success = request.httpResponseCode >= 200 && request.httpResponseCode < 300;
    ...
    if (success)
    {
        ...
        if (stateFlags.bootstrappingDeferred)
        {
            [OFBootstrapService
                doBootstrapWithNewAccount:NO
            ...
            for (id observer in priorityObservers)
            {
                if ([observer respondsToSelector:@selector(session:didLoginOnlyXpUser:)])
                    [observer session:self didLoginOnlyXpUser:currentUser];
            }
    ...
    [OpenFeint postXPUserSessionCompleted:success];
}
...
- (void)bootstrapSuccess:(OFPaginatedSeries*)responseObjects
{
    ...
    if (stateFlags.bootstrappingDeferred)
    {
        ...
        {
            if ([observer respondsToSelector:@selector(session:didLoginUser:previousUser:)])
                [observer session:self didLoginUser:[OpenFeint localUser] previousUser:nil];
        }
