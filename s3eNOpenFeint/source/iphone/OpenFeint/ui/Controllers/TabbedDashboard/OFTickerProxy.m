...
- (void)_kickTicker
{
	if ([OpenFeint isOnline])
	{
		...
		[OFTickerService getPage:1 
//                       onSuccess:OFDelegate(self, @selector(_tickerMessagesRecieved:)) 
//                       onFailure:OFDelegate()];
             onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_tickerMessagesRecieved:)]
