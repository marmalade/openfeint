...
@synthesize onApprovedInvocation = mOnApprovedInvocation;
@synthesize onDeniedInvocation = mOnDeniedInvocation;
...
- (void)serverDidTimeout
{
    ...
    {
        //self.viewControllers = [NSArray arrayWithObject:[[OFControllerLoaderObjC loader] load:@"UserFeintApproval"];// load(@"UserFeintApproval")];
        [self showOfflinePage:@"index"];
    }
}
...
{
    OFAssert(NO, @"Offline support disbaled for now...");
...
- (void)actionNavigateToURLWithUDID:(NSDictionary*)options
{
    ...
    targetURL = [targetURL stringByAppendingFormat:@"?udid=%@", [OpenFeint uniqueDeviceId]];
...
- (void)actionDismissNavController
{
    ...
    if (![OpenFeint hasUserSetFeintAccess])
    {
        ...
        [self.onDeniedInvocation invoke];
    }
...
- (void)actionBootstrap:(NSDictionary*)options
{
	...
	{
		[OpenFeint 
         doBootstrapAsNewUserOnSuccess:[OFInvocation invocationForTarget:self selector:@selector(bootstrapSuccess)]
         onFailure:[OFInvocation invocationForTarget:self selector:@selector(bootstrapFailed)]];
	}
	else
	{
		[OpenFeint 
         doBootstrapAsUserId:userId
         onSuccess:[OFInvocation invocationForTarget:self selector:@selector(bootstrapSuccess)]
         onFailure:[OFInvocation invocationForTarget:self selector:@selector(bootstrapFailed)]];        
	}
...
-(void) dealloc
{
    self.onApprovedInvocation = nil;
    self.onDeniedInvocation = nil;
    [super dealloc];
}
