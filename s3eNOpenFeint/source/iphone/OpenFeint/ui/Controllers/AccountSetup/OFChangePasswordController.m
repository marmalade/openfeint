...
static const int kOldPasswordInputFieldTag = 1;
static const int kPasswordInputFieldTag = 2;
static const int kPasswordConfirmationInputFieldTag = 3;
...
{
    [dataMap setObject:@"password" forKey:[NSNumber numberWithInt:kPasswordInputFieldTag]];
    [dataMap setObject:@"password_confirmation" forKey:[NSNumber numberWithInt:kPasswordConfirmationInputFieldTag]];
    [dataMap setObject:@"old_password" forKey:[NSNumber numberWithInt:kOldPasswordInputFieldTag]];
}
...
{
	[oldPassword release];
	UITextField* pwField = (UITextField*)[self.view findViewByTag:kOldPasswordInputFieldTag];
	oldPassword = [pwField.text retain];

	[password release];
	pwField = (UITextField*)[self.view findViewByTag:kPasswordInputFieldTag];
	password = [pwField.text retain];

	[passwordConfirmation release];
	pwField = (UITextField*)[self.view findViewByTag:kPasswordConfirmationInputFieldTag];
...
- (void)onFormSubmitted:(id)resources
{
	...
	if (password.length > 0 && passwordConfirmation.length > 0)
		controller.messageLabel.text = OFLOCALSTRING(@"Your password has been successfully changed!");
	else
		controller.messageLabel.text = OFLOCALSTRING(@"Your password has not been changed.");

	controller.messageTitleLabel.text = OFLOCALSTRING(@"Change Password");
