...
- (void)addHiddenParameters:(NSObject<OFISerializer>*)parameterStream
{
    ...
    [parameterStream ioNSStringToKey:@"credential_type" object:@"fbconnect"];
	
	if(updateServerPostingPermissions)
	{
        [parameterStream ioNSStringToKey:@"credential[has_extended_credentials]" object:@"true"];
	}
}
...
{
	updateServerPostingPermissions = NO;
}
...
{	
	...
	NSString* facebookApplicationKey = [[OFSettings instance] getSetting:@"facebook-application-key"];
	NSString* sessionProxy = [NSString stringWithFormat:@"%@fbconnect/get_session", [[OFSettings instance] getSetting:@"facebook-callback-url"]];
	OFFBSession* session = [OFFBSession sessionForApplication:facebookApplicationKey getSessionProxy:sessionProxy delegate:self];	
...
- (void)session:(OFFBSession*)session didLogin:(FBUID)uid
{
	self.fbuid = uid;
	...
	{
		OFFBPermissionDialog* dialog = [[[OFFBPermissionDialog alloc] init] autorelease];
...
- (void)dialog:(OFTFBDialog*)dialog didFailWithError:(NSError*)error
{
	[self displayError:[error localizedDescription]];
}
...
- (BOOL)dialog:(OFTFBDialog*)dialog shouldOpenURLInExternalBrowser:(NSURL*)url
{
