...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    UIButton *button = (UIButton*)[self.view findViewByTag:1];
...
- (void)onBootstrapFailure
{
    ...
    errorScreen.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(dismiss)] autorelease];
...
- (IBAction)_thisIsntMe
{
    ...
    accountFlowController.onCompletionInvocation = [OFInvocation invocationForTarget:self selector:@selector(continueFlow)];
...
- (void)selectUserWidget:(OFSelectUserWidget*)widget didSelectUser:(OFUser*)user
{
	...
	[OpenFeint doBootstrapAsUserId:user.resourceId
		onSuccess:[OFInvocation invocationForTarget:self selector:@selector(continueFlow)]
		onFailure:[OFInvocation invocationForTarget:self selector:@selector(onBootstrapFailure)]];
