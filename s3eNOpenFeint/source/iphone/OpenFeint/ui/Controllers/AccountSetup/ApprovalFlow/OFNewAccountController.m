...
@synthesize onCompletionInvocation, onCancelInvocation;

- (void)viewWillAppear:(BOOL)animated
{
    self.title = OFLOCALSTRING(@"New Account");
    
    UIButton *haveAccountButton = (UIButton*)[self.view findViewByTag:1];
	...
	[useDefaultNameButton setTitle:[NSString stringWithFormat:OFLOCALSTRING(@"Use %@"), [OpenFeint lastLoggedInUserName]] forState:UIControlStateNormal];
...
- (void)continueFlow
{
	if (closeDashboardOnCompletion)
	{
        ...
        {
			OFLinkSocialNetworksController* controller = (OFLinkSocialNetworksController*)[[OFControllerLoaderObjC loader] load:@"LinkSocialNetworks"];// load(@"LinkSocialNetworks");
            controller.onCompletionInvocation = self.onCompletionInvocation;
...
{
	OFOpenFeintAccountLoginController* accountFlowController = (OFOpenFeintAccountLoginController*)[[OFControllerLoaderObjC loader] load:@"OpenFeintAccountLogin"];// load(@"OpenFeintAccountLogin");
    accountFlowController.onCancelInvocation = [OFInvocation invocationForTarget:self selector:@selector(popBackToMe)];
    accountFlowController.onCompletionInvocation = [OFInvocation invocationForTarget:self selector:@selector(dismiss)];
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
	...
	[acceptNameButton setTitle:OFLOCALSTRING(@"Submit") forState:UIControlStateNormal];
	useDefaultNameButton.hidden = NO;
}

- (void)populateViewDataMap:(NSMutableDictionary*)dataMap
{
    [dataMap setObject:@"name" forKey:[NSNumber numberWithInt:nameEntryField.tag]];
}
...
{
    UIView *contentFrame = [self.view findViewByTag:101];
    contentFrame.hidden = YES;
    for (UIView *subview in [contentView subviews])
    {
		if(subview != feintLogoLargeView)
		{
			CGRect viewFrame = subview.frame;
			viewFrame.origin = CGPointMake(viewFrame.origin.x, viewFrame.origin.y - 5);
...
- (void)hideIntroFlowViews
{
    ...
    CGFloat verticalOffset = 32;
    if ([OFIntroNavigationController activeIntroNavigationController]) verticalOffset += 10;
    
    for (UIView *subview in [contentView subviews])
    {
        CGRect viewFrame = subview.frame;
        viewFrame.origin = CGPointMake(viewFrame.origin.x, viewFrame.origin.y - verticalOffset);
        subview.frame = viewFrame;
    }
    
	//Note: Tags 102, 103, and 104 only exist currently in the ipad xib, which means nothing happens here for these views on iphone.
    UIView *logo = [self.view findViewByTag:102];
    CGRect logoFrame = logo.frame;
    logoFrame.origin.y = logo.superview.frame.size.height - logoFrame.size.height;
    logo.frame = logoFrame;
    
    [self.view findViewByTag:103].hidden = YES;
    [self.view findViewByTag:104].hidden = YES;

	areIntroFlowViewsHidden = YES;
	[[NSNotificationCenter defaultCenter] postNotificationName:@"OFNSNotificationFullscreenFrameOn" object:nil];
...
{
    if (![OpenFeint isLargeScreen] || [OpenFeint isInLandscapeModeOniPad])
