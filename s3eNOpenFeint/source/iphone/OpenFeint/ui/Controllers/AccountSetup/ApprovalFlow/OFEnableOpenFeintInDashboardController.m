...
- (void)viewDidLoad
{
    appNameLabel.text = [NSString stringWithFormat:OFLOCALSTRING(@"%@ is Feint Enabled!"), [OpenFeint applicationDisplayName]];
}
...
//	OFDelegate success(self, @selector(_switchToOnlineMode));
	...
	[OpenFeint presentConfirmAccountModalInvocation:[OFInvocation invocationForTarget:self selector:@selector(_switchToOnlineMode)] useModalInDashboard:YES];
}

- (IBAction)clickedLeaderboards
{
	OFLeaderboardController* leaderboardController = (OFLeaderboardController*)[[OFControllerLoaderObjC loader] load:@"Leaderboard"];// load(@"Leaderboard");
