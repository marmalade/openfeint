...
{
	OFAnnouncementDetailController* detail = (OFAnnouncementDetailController*)[[OFControllerLoaderObjC loader] load:@"AnnouncementDetail"];// load(@"AnnouncementDetail");
...
{    
    self.thread = [OFAnnouncement announcementWithParams:params];
    self.thread.title = [params objectForKey:@"announcement_title"];
	self.topic = [OFForumTopic announcementsTopic];
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if (showCloseButton)
	{
		[segments addObject:OFLOCALSTRING(@"Close")];
	}
    ...
    if ([thread isKindOfClass:[OFAnnouncement class]])
    {
...
- (void)onSectionsCreated:(NSMutableArray*)sections
{
	...
	{
		// safe way to prune all resources but the header resource
		if ([sections count] > [self getHeaderResourceSectionIndex])
		{
			// remove all sections except the header section
			OFTableSectionDescription* headerSection = [[sections objectAtIndex:[self getHeaderResourceSectionIndex]] retain];
			[sections removeAllObjects];
			[sections addObject:headerSection];
			
			OFPaginatedSeries* resources = [headerSection page];
			if ([resources count])
			{
				// remove all objects except the first resource in this section
				id firstResource = [[resources objectAtIndex:0] retain];
				[[resources objects] removeAllObjects];
				[resources addObject:firstResource];
...
{
	OFSafeRelease(firstPost);
	firstPost = (OFForumPost*)[headerResource retain];
}
...
{
	NSString* message = OFLOCALSTRING(@"No one is discussing this announcement.");
	
	if (![OpenFeint allowUserGeneratedContent])
	{
		message = OFLOCALSTRING(@"Disabled by Parental Controls");
	}
	else if (thread.isLocked)
	{
		message = OFLOCALSTRING(@"Discussion has been disabled for this announcement.");
	}
