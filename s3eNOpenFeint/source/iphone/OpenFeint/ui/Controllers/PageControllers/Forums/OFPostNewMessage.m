...
{
	OFPostNewMessage* post = (OFPostNewMessage*)[[OFControllerLoaderObjC loader] load:@"PostNewMessage"];// load(@"PostNewMessage");
	post.topic = _topic;
...
- (void)viewDidAppear:(BOOL)animated
{
	...
	if (thread == nil)
	{
		self.title = OFLOCALSTRING(@"New Post");
		[subjectField becomeFirstResponder];
	}
	else
	{
		self.title = OFLOCALSTRING(@"Reply");
...
- (IBAction)onSubmitForm:(UIView*)sender
{
	...
	{
		[[[[UIAlertView alloc]
			initWithTitle:OFLOCALSTRING(@"Incomplete") 
			message:OFLOCALSTRING(@"Please enter a subject and try again.") 
			delegate:nil 
	...
	else if ([bodyField.text length] == 0)
	{
		[[[[UIAlertView alloc]
			initWithTitle:OFLOCALSTRING(@"Incomplete") 
			message:OFLOCALSTRING(@"Please enter a message body and try again.") 
			delegate:nil 
			cancelButtonTitle:OFLOCALSTRING(@"Ok") 
			otherButtonTitles:nil] autorelease] show];
	}
...
{
    OFForumThreadViewController *threadController = [self.navigationController.viewControllers objectAtIndex:[self.navigationController.viewControllers count]-2];
    if ([threadController isKindOfClass:[OFForumThreadViewController class]])
    {
        [threadController reloadDataFromServer];
    }    
...
- (void)addHiddenParameters:(NSObject<OFISerializer>*)parameterStream
{
	if (thread == nil)
	{
        [parameterStream ioNSStringToKey:@"discussion[subject]" object:subjectField.text];
        [parameterStream ioNSStringToKey:@"discussion[body]" object:bodyField.text];
	}
	else
	{
        [parameterStream ioNSStringToKey:@"post[body]" object:bodyField.text];
	}
}
