...
- (void)customLoader:(NSDictionary*)params
{
	...
	UIBarButtonItem* right = [[UIBarButtonItem alloc] initWithTitle:@"Invite" style:UIBarButtonItemStylePlain target:self action:@selector(advance)];
	self.navigationItem.rightBarButtonItem = right;
	[right release];
	
	self.inviteId = [params objectForKey:@"invite_id"];
	
	//If the string is empty, leave the inviteId as default - or nil value.
	if([self.inviteId isEqualToString:@""])
	{
		self.inviteId = nil;
	}
}
...
//	OFDelegate failure = OFDelegate(self, @selector(didFailGetInviteDefinition));
    OFInvocation* success = [OFInvocation invocationForTarget:self selector:@selector(didGetInviteDefinition:)];
    OFInvocation* failure = [OFInvocation invocationForTarget:self selector:@selector(didFailGetInviteDefinition)];
	
	//If this is not set, then they want the primary invite definition
	if(!self.inviteId)
	{
		[OFInviteService getDefaultInviteDefinitionForApplicationInvocation:success 
...
+ (OFSelectInviteTypeController*)inviteTypeControllerWithInviteIdentifier:(NSString*)inviteId
{
	...
	return (OFSelectInviteTypeController*)[[OFControllerLoaderObjC loader] load:@"SelectInviteType" withParams:params];
}
...
{
	OFSelectInviteTypeCell* curCell = (OFSelectInviteTypeCell*)[[OFControllerLoaderObjC loader] loadCell:@"SelectInviteType"];// loadCell(@"SelectInviteType");
...
- (void)advance
{
	...
	else
	{
		...
		OFInviteFriendsController* controller = (OFInviteFriendsController*)[[OFControllerLoaderObjC loader] load:@"InviteFriends"];// load(@"InviteFriends");
