...
- (void)populateResourceControllerMap:(NSMutableDictionary*)resourceMap
{
	...
	[resourceMap setObject:@"Contact" forKey:[OFDeviceContact class]];
}
...
- (void)doIndexActionOnSuccessInvocation:(OFInvocation*)success onFailureInvocation:(OFInvocation*)failure;
{
	...
	OFTableSectionDescription* section = [[[OFTableSectionDescription alloc] initWithTitle:OFLOCALSTRING(@"Selected Friends") andPage:page] autorelease];

    [success invokeWith:[OFPaginatedSeries paginatedSeriesWithObject:section]];
...
{
	return [NSString stringWithFormat:OFLOCALSTRING(@"All of your friends have %@.  Make some more!"), [OpenFeint applicationDisplayName]];
}
...
- (void)onTableHeaderCreated:(UIViewController*)tableHeader
{
	...
	header.titleLabel.text = [NSString stringWithFormat:OFLOCALSTRING(@"Invite Friends to play %@"), [OpenFeint applicationDisplayName]];
}

- (void)showNoFriendsSelectedMessage
{
	[[[[UIAlertView alloc] initWithTitle:OFLOCALSTRING(@"No Friends Selected") 
								 message:OFLOCALSTRING(@"You must select at least one friend to invite.") 
								delegate:nil 
...
{
	[[[[UIAlertView alloc] initWithTitle:OFLOCALSTRING(@"No internet connection") 
								 message:OFLOCALSTRING(@"You must have an internet connection to invite friends.") 
								delegate:nil 
					   cancelButtonTitle:OFLOCALSTRING(@"OK") 
					   otherButtonTitles:nil] autorelease] show];
}
...
{
	[[[[UIAlertView alloc] initWithTitle:OFLOCALSTRING(@"An error occurred") 
								 message:OFLOCALSTRING(@"Your invite was not submitted. Please try again.") 
								delegate:nil 
					   cancelButtonTitle:OFLOCALSTRING(@"OK") 
					   otherButtonTitles:nil] autorelease] show];
}
...
//		OFDelegate failure(self, @selector(inviteFailure));
		...
		[OFInviteService sendInvite:definition
                ...
                onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(inviteSuccess)]
                onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(inviteFailure)]];
...
- (void)inviteSuccess
{
	...
	OFInviteCompleteController* completeController = (OFInviteCompleteController*)[[OFControllerLoaderObjC loader] load:@"InviteComplete"];// load(@"InviteComplete");
	[completeController.inviteIcon setImageUrl:definition.inviteIconURL];

	if (selectedUsers.count > 1)
	{
		completeController.messageLabel.text = OFLOCALSTRING(@"Your invitations were sent!");
		completeController.messageTitleLabel.text = OFLOCALSTRING(@"Invitations Sent");
	}
	else
	{
		completeController.messageLabel.text = OFLOCALSTRING(@"Your invitation was sent!");
		completeController.messageTitleLabel.text = OFLOCALSTRING(@"Invitation Sent");
	}

	completeController.title = OFLOCALSTRING(@"Sending Invite");
...
{
	[super viewDidLoad];
	self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:OFLOCALSTRING(@"Send") style:UIBarButtonItemStylePlain target:self action:@selector(inviteFriends)] autorelease];
	self.title = OFLOCALSTRING(@"Compose Invite");
