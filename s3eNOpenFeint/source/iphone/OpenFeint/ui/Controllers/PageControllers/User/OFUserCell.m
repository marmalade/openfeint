...
OFLOCALIZECOMMENT("Bad localization practice: String built by pieces")
- (void)onResourceChanged:(OFResource*)resource
{
	...
	else
	{
        if ([user isLocalUser])
        {
            prefix = OFLOCALSTRING(@"You are playing");            
        }
        else
        {
            prefix = OFLOCALSTRING(@"Last played");
        }
