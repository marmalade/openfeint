...
static  NSString* kFollowingTabName = OFLOCALSTRING(@"Friends");
static  NSString* kFollowersTabName = OFLOCALSTRING(@"Pending");
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	self.title = [self localUsersFriends] ? OFLOCALSTRING(@"Feint Friends") : OFLOCALSTRING(@"Friends");
...
- (void)doIndexActionWithPage:(NSUInteger)pageIndex onSuccessInvocation:(OFInvocation*)success onFailureInvocation:(OFInvocation*)failure
{
	...
	{
		[OFFriendsService getAllUsersFollowedByUserAlphabetical:[self getPageContextUser].resourceId onSuccessInvocation:success onFailureInvocation:failure];	
	}
...
- (UIViewController*)getNoDataFoundViewController
{
	if ([self localUsersFriends] && friendCount == 0 && mCurTabType == kFollowingTab)
	{
		[self setEditing:NO];
		alwaysRefresh = YES;
		OFImportFriendsController* controller = (OFImportFriendsController*)[[OFControllerLoaderObjC loader] load:@"ImportFriends"];// load(@"ImportFriends");
...
- (NSString*)getNoDataFoundMessage
{
	...
	if (mCurTabType == kFollowersTab)
	{
		message = [self localUsersFriends]
			? OFLOCALSTRING(@"Here's where you'll find any pending friend requests.  You can choose to either add them to your Friends list or delete the request.")
			: OFLOCALSTRING(@"This user has no pending friends.");
	}
	else
	{
		message = [self localUsersFriends]
			? OFLOCALSTRING(@"You have not added any OpenFeint friends yet. Check the Pending tab to see who wants to friend you!")
			: OFLOCALSTRING(@"This user has not added any OpenFeint friends yet.");
	}
...
{
	refreshOnNextDidAppear = YES;
	[self.navigationController pushViewController:[[OFControllerLoaderObjC loader] load:@"ImportFriends"] /*load(@"ImportFriends")*/ animated:YES];	
...
- (UIView*)createPlainTableSectionHeader:(NSUInteger)sectionIndex
{
	...
	if (sectionIndex < [mSections count])
	{
		...
		headerView = [[OFControllerLoaderObjC loader] loadView:@"PlainTableSectionHeader"];// loadView(@"PlainTableSectionHeader");
...
//		OFDelegate failure(self, @selector(_failedRemoving));
        OFInvocation* failure = [OFInvocation invocationForTarget:self selector:@selector(_failedRemoving)];
		
		if (mCurTabType == kFollowersTab)
		{
//			OFDelegate success(self, @selector(_removedPendingFriend));
			[OFFriendsService removeLocalUsersFollower:user.resourceId 
                                   onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_removedPendingFriend)] 
                                   onFailureInvocation:failure];
		}
		else if (mCurTabType == kFollowingTab)
		{
//			OFDelegate success(self, @selector(_removedFriend:user:), user);
			[OFFriendsService makeLocalUserStopFollowing:user.resourceId 
                                     onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_removedFriend:user:) userParam:user] 
                                     onFailureInvocation:failure];
		}
	}
}
