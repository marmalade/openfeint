...
{
	[resourceMap setObject:@"ProfilePicture" forKey:[OFUsersCredential class]];
}
...
{
	[OFUsersCredentialService getProfilePictureCredentialsForLocalUserOnSuccessInvocation:success onFailureInvocation:failure];
}
...
//	OFDelegate failure(self, @selector(_refreshFailure));
	[OFUsersCredentialService getProfilePictureCredentialsForLocalUserOnSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_refreshSuccess:)]
                                                                      onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_refreshFailure)]];
}
...
//	OFDelegate failure(self, @selector(_refreshFailure));
	[OFUsersCredentialService requestProfilePictureUpdateForLocalUserOnSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_redownloadCredentials)]
                                                                     onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_refreshFailure)]]; 
}
...
- (void)onCellWasClicked:(OFResource*)cellResource indexPathInTable:(NSIndexPath*)indexPath
{
	...
	if ([credential isKindOfClass:[OFUsersCredential class]])
	{
		if (![credential isLinked] && ([credential isFacebook] || [credential isTwitter]))
		{
			...
			OFAccountSetupBaseController* controller = (OFAccountSetupBaseController*)[[OFControllerLoaderObjC loader] load:[OFUsersCredentialCell getCredentialControllerName:[credential credentialType]]];// load([OFUsersCredentialCell getCredentialControllerName:[credential credentialType]]);
        ...
        else
        {
			if ([credential.credentialType length] == 0)
			{
				...
				[OFUsersCredentialService
				 ...
				 onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_uploadSuccessWithIgnored:)]
				 onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_selectSourceFailure)]];
			}
			else if ([credential isHttpBasic])
			{
                ...
                OFImageView* imageView = cell.profilePictureView;
                popoverSourceFrame = imageView ? [self.tableView convertRect:imageView.frame fromView:cell] : CGRectZero;
			...
			else
			{
				...
				[OFUsersCredentialService
				 ...
				 onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_selectSourceSuccessWithIgnored:andCredential:) userParam:credential]
				 onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_selectSourceFailure)]];
			}
...
# ifdef __IPHONE_3_2
        ...
        [imagePickerPopoverController presentPopoverFromRect:popoverSourceFrame
...
{	
	...
	//On device version 3.1.3 if you do ANYTHING to flip this exactally over the x axis, there will be a bad line of pixels on the right side of the picture...
	CGContextRotateCTM(ctx, M_PI - .001f); 
	CGContextDrawImage(ctx, CGRectMake(0.0f, -50.f, 50.f, 50.f), image.CGImage);
