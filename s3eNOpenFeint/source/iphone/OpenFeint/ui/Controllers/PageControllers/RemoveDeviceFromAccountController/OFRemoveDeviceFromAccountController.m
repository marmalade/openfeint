...
{
    [parameterStream ioNSStringToKey:@"user_id" object:OpenFeint.lastLoggedInUserId];
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	{
		warningText.text = OFLOCALSTRING(@"You are about to remove your account from this device and log out. You will be able to log in to this account at any time with the email address and password you used to secure this account.");
	} else {
		warningText.text = OFLOCALSTRING(@"You are about to remove your account from this device and log out. You will not be able to recover your account unless you first secure it in the Settings tab!");
		
		NSString *errorMsg = OFLOCALSTRING(@"Warning! Your account will be lost and you will not be able to recover it. Tap 'Go Back' to return to the Settings tab and choose 'Enable Account Retrieval'.");
	...
	if (![OpenFeint isInLandscapeMode])
		titleLabel.text = OFLOCALSTRING(@"Remove Account");
}
