	...
	if (highScoresCnt > 0)
	{
		...
		for (unsigned int i = 0; i < highScoresCnt && annotationCount < maxAnnotations; i++)
		{
			...
			if ([[OpenFeint lastLoggedInUserId] isEqualToString:highScore.user.resourceId])
			{
				...
				[annotation setScore:highScore andLocation:YES];
			}
			else
			{
...
//	OFDelegate failure;
	...
	[OFHighScoreService getHighScoresFromLocation:location
                              ...
                              onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(loadAnnotationsFromHighScores:)] 
...
{
	static NSString* kHighScoreAnnotationViewReuseIdentifier = @"HighScoreAnnotationView";
