...
static const double milePerMeter = .000621371;
...
- (NSString *)title 
{
	...
	if( self.highScore )
	{
		...
		distanceBetweenScores = [userLocation OFdistanceFromLocation:highScoreLocation] * (unit == kDistanceUnitMiles ? milePerMeter : 1000);
        OFLOCALIZECOMMENT("miles or km, used as part of next string")
		distance = [NSString stringWithFormat:OFLOCALSTRING(@" [%.1f %@]"), distanceBetweenScores, (unit == kDistanceUnitMiles ? OFLOCALSTRING(@"miles") : OFLOCALSTRING(@"kms"))];
	}
	
    OFLOCALIZECOMMENT("This combines the name and the distance from above")
	title = [NSString stringWithFormat:OFLOCALSTRING(@"%@%@"), name, (distance ? distance : @"")];
...
- (NSString *)subtitle 
{
	...
	{
		subtitle = [NSString stringWithFormat:OFLOCALSTRING(@"Rank in area %d. %@"), self.highScore.rank, self.highScore.displayText];
	}
	else
	{
		subtitle = OFLOCALSTRING(@"No Score");
	}
	return subtitle;
}
