...
- (void)rejoinRoom
{
    [OFChatRoomInstanceService attemptToJoinRoom:roomInstance rejoining:YES 
                             onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(onRejoinedRoom)] 
                             onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(onFailedToRejoinRoom)]];
...
- (void)onFailedToRejoinRoom
{
	if (![OpenFeint isShowingErrorScreenInNavController:self.navigationController])
	{
		...
		[[[[UIAlertView alloc] initWithTitle:OFLOCALSTRING(@"Room Full!") message:OFLOCALSTRING(@"Sorry, you've lost your spot due to inactivity.") delegate:nil cancelButtonTitle:OFLOCALSTRING(@"OK") otherButtonTitles:nil] autorelease] show];
	}
}
...
{
    NSString* roomName = [params objectForKey:@"room_name"];
    NSString* roomType = [params objectForKey:@"room_type"];
    NSString* roomId = [params objectForKey:@"room_id"];
    if (roomId) {
        OFChatRoomInstance* chatRoom = [[[OFChatRoomInstance alloc] initWithId:roomId] autorelease];
        chatRoom.roomName = roomName;
        chatRoom.roomType = roomType;
        self.roomInstance = chatRoom;
        [OFChatRoomInstanceService attemptToJoinRoom:roomInstance rejoining:NO 
                                 onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(onRejoinedRoom)] 
                                 onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(onFailedToRejoinRoom)]];
    }
    mIsCustomLoad = YES;
...
{	
	[super viewDidLoad];
	mChatRoomMessageBoxController = [(OFChatRoomMessageBoxController*)[[OFControllerLoaderObjC loader] load:@"ChatRoomMessageBox"] retain];
}
...
{
	[resourceMap setObject:@"ChatRoomChatMessage" forKey:[OFChatMessage class]];
}
...
{
	return [NSString stringWithFormat:OFLOCALSTRING(@"We're downloading messages in the background or no one has said anything yet.")];
}
