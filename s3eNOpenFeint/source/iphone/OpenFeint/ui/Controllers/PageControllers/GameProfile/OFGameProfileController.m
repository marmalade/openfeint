...
+ (void)showGameProfileWithClientApplicationId:(NSString*)clientApplicationId compareToUser:(OFUser*)comparisonUser
{
	...
	OFGameProfileController* controller = (OFGameProfileController*)[[OFControllerLoaderObjC loader] load:xibName];// load(xibName);
...
{
	UIViewController* controllerToPush = [[OFControllerLoaderObjC loader] load:controllerName];// load(controllerName);
...
- (IBAction)pressedEnableOpenFeint
{
    ...
    [OpenFeint addBootstrapInvocations:[OFInvocation invocationForTarget:self selector:@selector(_switchToOnlineMode)] 
                   onFailure:[OFInvocation invocationForTarget:self selector:@selector(_stayInOfflineMode)]];
	
//	[OpenFeint addBootstrapInvocations:OFDelegate(self, @selector(_switchToOnlineMode)) onFailure:OFDelegate(self, @selector(_stayInOfflineMode))];

//	OFDelegate success;
//	OFDelegate failure(self, @selector(hideLoadingScreen));
	[OpenFeint presentUserFeintApprovalModalInvocation:nil deniedInvocation:[OFInvocation invocationForTarget:self selector:@selector(hideLoadingScreen) ]];
}
...
- (void)_downloadGameProfileInfo
{
	...
	[OFClientApplicationService getGameProfilePageInfo:clientApplicationId 
                                   onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_downloadedGameProfile:)]
                                   onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_failedDownloadingGameProfile)]];
