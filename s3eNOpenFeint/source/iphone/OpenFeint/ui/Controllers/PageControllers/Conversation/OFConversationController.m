...
- (void)customLoader:(NSDictionary*)params
{
    ...
    if (userId)
    {
        [OFUserService getUser:userId 
           onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(getUserSuccess:)] 
           onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(getUserFailure)]];
    }
}

- (void)getUserSuccess:(OFPaginatedSeries*)pages
{
    NSObject* obj = [[pages objects] objectAtIndex:0];
    if ([obj isKindOfClass:[OFUser class]])
    {
        self.conversationUser = (OFUser*)obj;
		...
		[OFConversationService 
         ...
         onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_startedConversation:)]
         onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_errorStartingConversation)]];
    }
}
...
- (void)_initialIndexComplete:(OFPaginatedSeries*)resources nextCall:(OFInvocation*)nextCall
{
...
- (void)_startedConversation:(OFPaginatedSeries*)conversationPage
{
	if ([conversationPage count] == 1)
	{
        ...
        // The message box may be hidden while we are loading the conversation so the user
        // can't post a message too early.  Unhide it now.
        messageBox.view.hidden = NO;
...
- (void)loadView
{
	...
	self.messageBox = (OFConversationMessageBoxController*)[[OFControllerLoaderObjC loader] load:@"ConversationMessageBox" owner:self];
	messageBox.conversationId = conversationId;
    
    if (!conversationUser)
    {
        // When we are loading with only a user id, and not an OFUser,
        // we need to hide the keyboard until everything is loaded in.
        messageBox.view.hidden = YES;
    }
}
...
{
	[resourceMap setObject:@"Conversation" forKey:[OFForumPost class]];
    [resourceMap setObject:@"OnlineStatus" forKey:[OFOnlineStatus class]];
}
...
//		OFDelegate chainedSuccess(self, @selector(_initialIndexComplete:nextCall:), success);
		...
		[OFConversationService
         ...
         page:1
         onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_initialIndexComplete:nextCall:) chained:success] 
         onFailureInvocation:failure];
	}
	else if (self.conversationUser != nil)
	{
		[OFConversationService 
         startConversationWithUser:self.conversationUser.resourceId 
         onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_startedConversation:)]
