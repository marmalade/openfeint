...
- (void)setControlToSettingValue:(OFUserSetting*)setting
{
	if([setting.valueType isEqualToString:@"bool"])
	{
		...
		booleanSwitch.enabled = YES;
	}
	else
	{
		booleanSwitch.enabled = NO;
		booleanSwitch.on = NO;
	}
}
...
- (IBAction)switchToggled
{
	...
	[OFUserSettingService 
     ...
     onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(onSubmitSuccess)]
     onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(onSubmitFailure)]];
