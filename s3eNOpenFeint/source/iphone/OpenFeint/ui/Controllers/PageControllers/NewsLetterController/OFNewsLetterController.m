...
- (void)setSubscriptionState:(BOOL)subscribe
{
	...
	[OFUserSettingService setSubscribeToDeveloperNewsLetter:subscriptionSwitch.on 
                                        ...
                                        onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(onSubscriptionChanged:)]
                                        onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(onSubscriptionFailedToChange)]];
...
- (IBAction)subscriptionChanged
{
	...
	if (subscriptionSwitch.on && ![OpenFeint loggedInUserHasHttpBasicCredential])
	{
		...
		UIActionSheet* actionSheet = [[[UIActionSheet alloc] initWithTitle:OFLOCALSTRING(@"To subscribe you must first enable account retrieval with an e-mail and password.") delegate:self cancelButtonTitle:OFLOCALSTRING(@"Cancel") destructiveButtonTitle:OFLOCALSTRING(@"Enable Account Retrieval") otherButtonTitles:nil] autorelease];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
	...
	{
		securingAccount = YES;
		OFHttpCredentialsCreateController* httpController = (OFHttpCredentialsCreateController*)[[OFControllerLoaderObjC loader] load:@"HttpCredentialsCreate"];// load(@"HttpCredentialsCreate");
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if (![OpenFeint isInLandscapeMode])
		self.summaryLabel.text = OFLOCALSTRING(@"Subscribe to new updates!");
	self.descriptionLabel.text = [NSString stringWithFormat:
		OFLOCALSTRING(@"Subscribe to email updates from the developer of %@.  Don't worry, your email address is stored securely and won't be shared with any third parties."),
		[OpenFeint applicationDisplayName]];
}

- (void)doServerAction
{
	[OFUserSettingService getSubscribingToDeveloperNewsLetter:[self getClientApplicationId] 
                                          onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(onRetrievedSetting:)]
                                          onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(onFailedRetrievingSetting)]];
//                                                    onSuccess:OFDelegate(self, @selector(onRetrievedSetting:)) 
//                                                    onFailure:OFDelegate(self, @selector(onFailedRetrievingSetting))];
}
