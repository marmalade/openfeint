...
@interface OFSendChallengeHeaderController ()
@property (nonatomic, retain) NSObject* selfRetainer;
@property (nonatomic, retain) NSObject* parentRetainer;
@end


@implementation OFSendChallengeHeaderController
@synthesize selfRetainer = mSelfRetainer;
@synthesize parentRetainer = mParentRetainer;
...
- (void)awakeFromNib
{
	...
	{
		self.titleLabel.text = OFLOCALSTRING(@"Loading...");
...
- (void)dealloc
{
    ...
    self.selfRetainer = nil;  //not sure how this line would actually do anything...
    self.parentRetainer = nil;
	[super dealloc];
}
...
- (void)viewDidLoad
{
	...
	self.parentRetainer = sendChallengeController;
...
//	OFDelegate failure(self, @selector(_challengeDefinitionDownloadFailed));
	[OFChallengeDefinitionService getChallengeDefinitionWithId:sendChallengeController.challengeDefinitionId 
                                           onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(_newChallengeDefinitionDownloaded:)]
                                           onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(_challengeDefinitionDownloadFailed)]];
//                                                     onSuccess:success 
//                                                     onFailure:failure];	
...
{
	[[[[UIAlertView alloc] initWithTitle:OFLOCALSTRING(@"Error Occurred") 
								 message:OFLOCALSTRING(@"OpenFeint failed downloading data. Please check your connection and try again.") 
								delegate:nil 
