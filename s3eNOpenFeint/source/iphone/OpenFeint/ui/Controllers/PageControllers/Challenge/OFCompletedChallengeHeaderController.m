...
{	
	...
	BOOL showUserMessageTextField = NO;
	if (newUserChallenge.challenge.challengeDefinition.multiAttempt)
	{
		if (newUserChallenge.result != kChallengeResultRecipientWon)
		{
			[self.actionButton setTitleForAllStates:OFLOCALSTRING(@"Try Again")];
			action = @selector(tryAgainPressed);
			target = self;
		}
		else
		{
			[self.actionButton setTitleForAllStates:OFLOCALSTRING(@"Send Result As Challenge")];
			action = @selector(submitChallengeBack);
			showUserMessageTextField = YES;
	...
	{
		[self.actionButton setTitleForAllStates:OFLOCALSTRING(@"OK")];
