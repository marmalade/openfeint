...
{
	[resourceMap setObject:@"SelectableUser" forKey:[OFUser class]];
}
...
- (void)doIndexActionWithPage:(NSUInteger)pageIndex onSuccessInvocation:(OFInvocation*)success onFailureInvocation:(OFInvocation*)failure
{
	...
	[OFChallengeService getUsersToChallenge:(NSString*)instigatingChallengerId pageIndex:pageIndex onSuccessInvocation:success onFailureInvocation:failure];
}
...
{
    OFLOCALIZECOMMENT("Built string")
	return [NSString stringWithFormat:OFLOCALSTRING(@"You have no friends with %@."), [OpenFeint applicationDisplayName]];
}
...
- (void)onSectionsCreated:(NSMutableArray*)sections
{
	if (!isCompleted && [sections count] == 1)
	{
		...
		firstSection.title = OFLOCALSTRING(@"Friends To Challenge");
	}
}
...
- (void)onCell:(OFUserCell*)cell resourceChanged:(OFUser*)user
{
...
- (void)showNoFriendsSelectedMessage
{
	[[[[UIAlertView alloc] initWithTitle:OFLOCALSTRING(@"No Friends Selected") 
								 message:OFLOCALSTRING(@"You must select at least one friend to challenge.") 
								delegate:nil 
...
- (void)showNoConnectionMessage
{
	[[[[UIAlertView alloc] initWithTitle:OFLOCALSTRING(@"No internet connection") 
								 message:OFLOCALSTRING(@"You must have an internet connection to submit challenges.") 
								delegate:nil 
					   cancelButtonTitle:OFLOCALSTRING(@"OK") 
					   otherButtonTitles:nil] autorelease] show];
}
...
- (void)showSubmitFailedMessage
{
	[[[[UIAlertView alloc] initWithTitle:OFLOCALSTRING(@"An error occurred") 
								 message:OFLOCALSTRING(@"Your challenge was not submitted. Please try again.") 
								delegate:nil 
					   cancelButtonTitle:OFLOCALSTRING(@"OK") 
					   otherButtonTitles:nil] autorelease] show];
}
...
//		OFDelegate failure(self, @selector(challengeSubmittedFailure));
		...
		[OFChallengeService sendChallenge:self.challengeDefinitionId
					...
					inResponseToChallenge:nil
                      onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(challengeSubmittedSuccess)]
                      onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(challengeSubmittedFailure)]];
	}
}
