...
{
	OFFanClubCell* curCell = (OFFanClubCell*)[[OFControllerLoaderObjC loader] loadCell:@"FanClub"];// loadCell(@"FanClub");
...
- (void)onCellWasClicked:(OFResource*)cellResource indexPathInTable:(NSIndexPath*)indexPath
{
	if ([mSections count] > 0)
	{
		...
		if ([staticCells count] > indexPath.row)
		{
			...
			{
				[self.navigationController pushViewController:[[OFControllerLoaderObjC loader] load:cell.controllerName]/* load(cell.controllerName)*/ animated:YES];
			}
		}
	}
}
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if ([mSections count] > 0 && [[self getPageContextGame] isLocalGameInfo])
	{
		...
		OFAssert(NO, @"I can't find the dev announcements cell, so I don't know where to put the badge!");
	}	
}
