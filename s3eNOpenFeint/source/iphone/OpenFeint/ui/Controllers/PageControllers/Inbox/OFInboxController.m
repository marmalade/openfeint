...
{
	return OFLOCALSTRING(@"Your inbox is empty.");
}
...
- (void)_newMessagePressed
{
    ...
    [OFFriendPickerController launchPickerWithDelegate:self promptText:OFLOCALSTRING(@"Select a friend to message")];
}
...
{
	[resourceMap setObject:@"Inbox" forKey:[OFSubscription class]];
}
...
{
    if([self cellTypeAtIndexPath:indexPath] == OFTableSectionCellTypeAction) return;
...
-(void)actionSheet:(UIActionSheet*)sheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if(buttonIndex != [sheet cancelButtonIndex]) {
        ...
        [OFConversationService deleteConversation:sub.discussionId 
                              onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(deletionWasSuccessful)]
...
- (void) actionPressedForTag:(NSUInteger) tag indexPath:(NSIndexPath*) indexPath {
    ...
    if(tag == 1) {        
        ...
        UIActionSheet* sheet = [[UIActionSheet alloc] initWithTitle:OFLOCALSTRING(@"Deleting IM") 
                                                           delegate:self 
                                                  cancelButtonTitle:OFLOCALSTRING(@"Cancel") 
                                             destructiveButtonTitle:OFLOCALSTRING(@"Delete") 
