...
- (void)setReviewAreaAvailable:(BOOL)available animate:(BOOL)animate
{
	...
	else
	{
		self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] 
												   initWithTitle:OFLOCALSTRING(@"Submit") 
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	[OFClientApplicationService getPlayerReviewForGame:[self getClientApplicationId] 
												byUser:nil 
                                   onSuccessInvocation:[OFInvocation invocationForTarget:self selector:@selector(onReviewLoaded:)]
                                   onFailureInvocation:[OFInvocation invocationForTarget:self selector:@selector(onRequestFailed)]];
...
{
	isFavorite = YES;
    NSString* buttonImageName = ([OpenFeint isLargeScreen]) ? @"OFFavoriteThisGameIconRateOn.png" : @"OFButtonStarFilled.png";
    [self setButtonImage:buttonImageName];
	self.reviewTextField.text = review;
	self.descriptionLabel.text = [NSString stringWithFormat:OFLOCALSTRING(@"You've added %@ to your Favorites. From now on when your friends view your list of games, this one will be marked with a star. Press the star again to remove it from your Favorites."), [self getClientApplicationName]];
...
{
	isFavorite = NO;
	self.descriptionLabel.text = OFLOCALSTRING(@"Love the game? Press the star below to make it a Favorite. You'll help support the developer and share your Favorites with your friends.");
    NSString* buttonImageName = ([OpenFeint isLargeScreen]) ? @"OFFavoriteThisGameIconRateOff.png" : @"OFButtonStarEmpty.png";
    [self setButtonImage:buttonImageName];
...
- (void)updateState:(BOOL)includeText makeFavorite:(BOOL)makeFavorite successInvocation:(OFInvocation*)success
{
	[self showLoadingScreen];
	if (makeFavorite)
	{
...
{
	[self hideLoadingScreen];
	[[[[UIAlertView alloc] initWithTitle:nil message:OFLOCALSTRING(@"Your comment has been submitted.") delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] autorelease] show];
}

- (IBAction) onFavoritePressed
{
	[self updateState:NO makeFavorite:!isFavorite successInvocation:[OFInvocation invocationForTarget:self selector:@selector(onIsFavoriteChanged)]];
...
- (IBAction) onSubmitReviewPressed
{
	...
	{
		[self updateState:YES makeFavorite:YES successInvocation:[OFInvocation invocationForTarget:self selector:@selector(onReviewSubmitted)]];
//		[self updateState:YES makeFavorite:YES successDelegate:OFDelegate(self, @selector(onReviewSubmitted))];
	}
	else
	{
		[[[[UIAlertView alloc] initWithTitle:nil message:OFLOCALSTRING(@"You must enter a comment before submitting.") delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] autorelease] show];
	}
}
...
- (void)addHiddenParameters:(NSObject<OFISerializer>*)parameterStream
{
	...
	{
        [parameterStream pushScope:@"client_application_user" isArray:NO];
		if ([self.reviewTextField.text length] > 0)
		{
            [parameterStream ioNSStringToKey:@"review" object:self.reviewTextField.text];
		}
		BOOL favorite = isFavorite;
        [parameterStream ioBoolToKey:@"favorite" value:favorite];
        [parameterStream popScope];
...
- (void)onFormSubmitted:(id)resources
{
	...
	UIAlertView* alertSheet = [[[UIAlertView alloc] initWithTitle:nil
														  message:OFLOCALSTRING(@"Your comment has been submitted successfully.") 
														 delegate:nil 
