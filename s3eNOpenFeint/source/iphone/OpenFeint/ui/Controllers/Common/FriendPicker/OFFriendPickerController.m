...
+ (void)launchPickerWithDelegate:(id<OFFriendPickerDelegate>)_delegate promptText:(NSString*)_promptText mustHaveApplicationId:(NSString*)_applicationId
{
	...
	OFFriendPickerController* picker = (OFFriendPickerController*)[[OFControllerLoaderObjC loader] load:@"FriendPicker"];// load(@"FriendPicker");
...
{
    return [OFInvocation invocationForTarget:self selector:@selector(_customFailHandler)];
}
...
- (void)doIndexActionOnSuccessInvocation:(OFInvocation*)success onFailureInvocation:(OFInvocation*)failure;
{
	...
	{
		[OFFriendsService getAllUsersWithApp:scopedByApplicationId followedByUser:nil 
             alphabeticalOnSuccessInvocation:success onFailureInvocation:failure];
	}
	else
	{
		[OFFriendsService getAllUsersFollowedByLocalUserAlphabeticalInvocation:success 
