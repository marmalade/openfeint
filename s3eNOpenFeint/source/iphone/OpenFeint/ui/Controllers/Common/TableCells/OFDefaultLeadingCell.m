...
static const float kIconPadding = 10.f;
...
{
    if (mCallbackTarget && mRightIconSelector) {
        [mCallbackTarget performSelector:mRightIconSelector];
    }
}
...
{
    mRightIconSelector = rightIconSelector;
}
...
- (void)populateRightIconsAsComparisonWithImageUrl:(NSString*)imageUrl
{
	[self enableRightIconView];
    [self enableSecondFromRightIconView];
    [self.secondFromRightIconView useProfilePictureFromUser:[OpenFeint localUser]];		
    [self.rightIconView setImageUrl:imageUrl];
}
...
- (void)populateRightIconsAsComparison:(OFUser*)pageOwner
{
	...
	if (myPage)
	{
		[self.rightIconView useProfilePictureFromUser:[OpenFeint localUser]];
