...
+ (id)controllerWithTitle:(NSString*)title htmlPath:(NSString*)htmlPath useContentFrame:(BOOL)useContentFrame
{
    return [[[self alloc] initWithTitle:title htmlPath:htmlPath useContentFrame:useContentFrame] autorelease];
}
...
- (id)initWithTitle:(NSString*)aTitle htmlPath:(NSString*)htmlPath useContentFrame:(BOOL)_useContentFrame
{
    ...
    if (self)
    {
        ...
        if (![OFWebViewManifestService trackPath:htmlPath forMe:self]) {
            // All is loaded and ready
            NSString *absolutePath = [[OFWebViewManifestService rootPath] stringByAppendingPathComponent:htmlPath];
            self.url = [NSURL fileURLWithPath:absolutePath];
        }
...
- (void)viewDidLoad
{
    ...
    if (url) [self loadInitialRequest];
	...
	//Setup a close button initially incase the use closes before loading finishes.
	UIBarButtonItem* navBarButton = [[[UIBarButtonItem alloc] initWithTitle:@"Close"
																	  style:UIBarButtonItemStyleBordered
																	 target:self
																	 action:@selector(actionDismissNavController)] autorelease];
	self.topViewController.navigationItem.rightBarButtonItem = navBarButton;
}

- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    if (!url) [self showLoadingIndicator];
}

- (void)webViewCacheItemReady:(NSString*)path {
    NSString *absolutePath = [[OFWebViewManifestService rootPath] stringByAppendingPathComponent:path];
    self.url = [NSURL fileURLWithPath:absolutePath];
...
- (void)loadInitialRequest
{
    NSMutableArray *params = [NSMutableArray arrayWithObjects:
                              ...
                              [[[MPURLRequestParameter alloc] initWithName:@"server_url" andValue:[[OFSettings instance] getSetting:@"server-url"]] autorelease],
                              nil];
    
    NSMutableArray *paramStrings = [NSMutableArray array];
    for (MPURLRequestParameter *param in params) {
        [paramStrings addObject:[param URLEncodedParameterString]];
    }
    
    NSString *urlString = [url absoluteString];
    urlString = [urlString stringByAppendingFormat:@"?%@", [paramStrings componentsJoinedByString:@"&"]];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (useContentFrame && ![viewController isKindOfClass:[OFWebNavContentController class]]) {
        UIScrollView* topScrollView = [viewController.view findFirstScrollView];
...
- (void)performAction:(NSURL*)actionURL
{
    ...
    if (![name isEqualToString:@"/log"]) OFLog(@"ACTION: %@", actionURL);
    
    NSInvocation *invocation = [actionMap objectForKey:name];
    if (invocation) {
        // Always has at least 2 args. 3 means method takes one argument.
...
- (void)didTapBarButton
{
    ...
    OFLOCALIZECOMMENT("Next two 'Done's need to be same, used for an equals check")
    if ([barButton.title isEqualToString:@"Edit"])
    {
        barButton.title = OFLOCALSTRING(@"Done");
        barButton.style = UIBarButtonItemStyleDone;
    }
    else if ([barButton.title isEqualToString:OFLOCALSTRING(@"Done")])
    {
        barButton.title = OFLOCALSTRING(@"Edit");
...
{
    UIViewController *controller = [[OFControllerLoaderObjC loader] load:controllerName withParams:options];// loadWithParamsDictionary(controllerName, options, nil);
...
// Log something to the XCode console
- (void)actionLog:(NSDictionary*)options
{
    NSString *message = [options objectForKey:@"message"];
    if (message) OFLog(@"WEBLOG: %@", message);
}
