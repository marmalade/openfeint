...
typedef struct {
    ...
    CGRect navBarFrame;
    CGRect bottomFrame;
    OFFramedNavigationControllerVisibilityFlags visibilityFlags;
} OFFrameRectangleInfo;
...
- (void) _getRectsForFrameInfo:(OFFrameRectangleInfo*) frameInfo;
@end
...
@synthesize bannerView, frameView, shouldHandleFullscreenFrame;

-(OFTabBarItem*) owningTabBarItem {
    return owningTabBarItem;
}

-(void) setOwningTabBarItem:(OFTabBarItem *) tabItem {
    if(tabItem != self.owningTabBarItem) {
        owningTabBarItem = tabItem;
    }
    if([tabItem.viewController isKindOfClass:[OFNavigationController class]]) {
        OFNavigationController* navController = (OFNavigationController*) tabItem.viewController;
        if([navController.visibleViewController respondsToSelector:@selector(setOwningTabBarItem:)]) {
            [(id)navController.visibleViewController setOwningTabBarItem:tabItem];
        }
    }
    
}

- (void)setShouldHandleFullscreenFrame:(BOOL)should
{
	if (should != shouldHandleFullscreenFrame)
	{
		shouldHandleFullscreenFrame = should;
...
- (OFBannerCell*)loadBannerFromController:(UIViewController*)controller
{
	...
	if ([controller conformsToProtocol:@protocol(OFBannerProvider)])
	{
		...
		{				
			...
			UITableViewCell* hopefullyAnOFBannerCell = [[OFControllerLoaderObjC loader] loadCell:bannerCellControllerName];// loadCell(bannerCellControllerName, nil);
			if (hopefullyAnOFBannerCell)
			{
				if ([hopefullyAnOFBannerCell isKindOfClass:[OFBannerCell class]])
				{
					...
					bannerCell.bannerProvider = (UIViewController<OFBannerProvider>*)controller;
...
- (void)showLoadingIndicator
{
	...
	if (shouldHandleFullscreenFrame)
	{
		[self.frameView addSubview:mLoadingController.view];
		[self.frameView sendSubviewToBack:mLoadingController.view];
	}
	else
	{
...
- (void)viewDidLoad
{
	...
	OFContentFrameView* cfv = (OFContentFrameView*)[[OFControllerLoaderObjC loader] loadView:@"ContentFrame"];// loadView(@"ContentFrame");
    cfv.roundedCorners = NO;
	cfv.frame = frameView.bounds;
	[frameView addSubview:cfv];
	
	OFSafeRelease(bannerView);
	bannerView = [(OFBannerFrame*)[[OFControllerLoaderObjC loader] loadView:@"BannerFrame"] retain]; //loadView(@"BannerFrame") retain];
...
- (void)setOnlineStatus:(BOOL)onlineStatus {
    ...
    for(UIViewController* controller in self.viewControllers) {
        if([controller respondsToSelector:@selector(setOnlineStatus:)]) {
            [(id)controller setOnlineStatus:onlineStatus];
        }
    }
}
...
- (void)_pushViewController:(UIViewController*)controller animated:(BOOL)animated inContextOfUser:(OFUser*)userContext comparedToUser:(OFUser*)comparedToUser
{
	isPushingOrPopping = YES; 
	[self.view bringSubviewToFront:self.navigationBar];
	
	BOOL unframed = (![(id)controller framedNavigationControllerShouldApplyFrame:self]);
	frameView.hidden = unframed; // @TODO animate
	
	if (!unframed && ![controller.view isKindOfClass:[OFFramedContentWrapperView class]])
	{
...
- (void)changeUserContext:(OFUser*)user
{
    [self _changeUserTo:user comparedWith:[self comparisonUser] notifyController:NO];
    [self refreshBanner];
}
...
- (void)refreshBottomView
{
	...
	{
		newBottomView = [(UIViewController<OFCustomBottomView>*)self.topViewController getBottomView];
	}
	...
	if (customBottomView)
	{
		...
		//Adding this custom bottom view to the parent view on a 2.x iphone device causes the next
		//screen to be loaded to be unresponsive.  This is because the UITrainsitionView (which is
		//the view responsible for animated from one screen to another) never destroys itself, and
		//intercepts all of the touches.  Adding this view as a subclass of the controller's view
		//doesn't cause this problem on a 2.x device.
		if(is2PointOhSystemVersion())
		{
		...
		{
			//This code is specificly for ipad, since the dashboard is smaller than the device display,
			//if the user rotates the device while in the middle of a dashboard flow, we need to rotate
			//the daashboard.  Since the bug described above only happens on 2.x, i.e. iphone/itouch,
			//where we don't support flipping the dashboard in the middle of a flow, this is a valid fix.
			UIView *parentView = [self.view superview];
			if ([parentView isKindOfClass:[UIWindow class]]) parentView = self.view;
			
			[parentView addSubview:customBottomView];
			[parentView bringSubviewToFront:customBottomView];
		}
...
- (OFFramedNavigationControllerVisibilityFlags)_visibilityFlagsForController:(UIViewController*)viewController
{
	...
	if ([viewController shouldAlwaysShowNavBar])
	{
		v.showNavBar = YES;
	}
	else
...
- (void)_getRectsForFrameInfo:(OFFrameRectangleInfo*)frameInfo
{
	...
	frameInfo->navBarFrame.origin.y = frameInfo->visibilityFlags.showNavBar ? 0.f : -frameInfo->navBarFrame.size.height;
	
	// base banner off nav bar.
	if ([OpenFeint isLargeScreen]) {
        frameInfo->bannerFrame.size = CGSizeMake(640.f, 78.f);
    } else {
        frameInfo->bannerFrame.size = CGSizeMake([OpenFeint isInLandscapeMode] ? 480.f : 320.f, 58.f);
    }
	frameInfo->bannerFrame.origin = CGPointZero;
	frameInfo->bannerFrame.origin.y = CGRectGetMaxY(frameInfo->navBarFrame);
	frameInfo->bannerFrame.origin.y -= frameInfo->visibilityFlags.showBanner ? 0.f : frameInfo->bannerFrame.size.height;
	
	// base the frame and content insets off the banner.
	frameInfo->frameFrame.origin = CGPointZero;
	frameInfo->frameFrame.origin.y = CGRectGetMaxY(frameInfo->bannerFrame);
	frameInfo->frameFrame.origin.y -= frameInfo->visibilityFlags.showBanner ? kfBannerOverlap : 0.f;  // we want them overlapping slightly
	
	frameInfo->frameFrame.size.height -= frameInfo->frameFrame.origin.y;
    frameInfo->frameFrame.size.height -= isKeyboardShown ? keyboardHeight : 0;
	frameInfo->frameFrame.size.height -= [self bottomPadding];
    
    if ([OpenFeint isLargeScreen])
    {
        frameInfo->frameFrame.size.height += 7.f;
    }
	
	if (frameInfo->visibilityFlags.showBottomView)
	{
        frameInfo->bottomFrame.origin.y = CGRectGetMaxY(frameInfo->frameFrame) - frameInfo->bottomFrame.size.height;
		frameInfo->frameFrame.size.height -= frameInfo->bottomFrame.size.height;
	}
}
...
- (void)_repositionViews
{
    ...
    OFFrameRectangleInfo info = {
        bannerView.frame,
        frameFrame,
        self.navigationBar.frame,
        customBottomView.frame,
        visibilityDoes
    };
    
    [self _getRectsForFrameInfo:&info];
	
	if (!is2PointOhSystemVersion())
	{
		self.navigationBar.frame = info.navBarFrame;
	}
	frameView.frame = info.frameFrame;
	bannerView.frame = info.bannerFrame;
	customBottomView.frame = info.bottomFrame;
}
...
- (UIEdgeInsets)_calculateInsetsFor:(UIViewController*)viewController
{
    ...
    OFFrameRectangleInfo info = {
        ...
        frameFrame,
        self.navigationBar.frame,
        customBottomView ? customBottomView.frame : CGRectZero,
        [self _visibilityFlagsForController:viewController]
    };
	...
	if (is2PointOhSystemVersion())
	{
		...
		CGRect frameInViewControllerSpace = [self.view convertRect:info.frameFrame toView:viewController.view];
	...
	contentInsets.top += info.frameFrame.origin.y;
	...
	{
		contentInsets.top -= info.navBarFrame.size.height;
	}
	...
	if (is2PointOhSystemVersion())
	{
		if (self.isNavigationBarHidden)
		{
			contentInsets.bottom -= info.navBarFrame.size.height;
		}
	}
	...
	if (info.visibilityFlags.showBottomView)
	{
		contentInsets.bottom += info.bottomFrame.size.height;
	}
...
- (void)_changeUserTo:(OFUser*)user comparedWith:(OFUser*)comparedWithUser notifyController:(BOOL)notifyController
{
	...
	if (fci.comparedToUser != comparedWithUser)
	{
		changedComparedUser = YES;
		fci.comparedToUser = comparedWithUser;
	}

	if ((changedUser || changedComparedUser) && notifyController && 
		[fci.controller respondsToSelector:@selector(profileUsersChanged:comparedToUser:)])
	{
