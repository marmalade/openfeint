...
- (void)onResourceChanged:(OFResource*)resource
{
	...
	else
	{
		...
		if ([user isLocalUser])
		{
			nowPlayingLabel.text = [NSString stringWithFormat:OFLOCALSTRING(@"Playing %@"), user.lastPlayedGameName];
		}
		else if ([user online]) 
		{
			nowPlayingLabel.text = [NSString stringWithFormat:OFLOCALSTRING(@"playing %@"), user.lastPlayedGameName];
			...
			onlineStatusLabel.text = OFLOCALSTRING(@"ONLINE");
		} 
		else 
		{
			nowPlayingLabel.text = [NSString stringWithFormat:OFLOCALSTRING(@"Last played %@"), user.lastPlayedGameName];
		}
