...
@image html of_devSupport.png 
			...
			<h3>Platform: OpenFeint iOS SDK 2.12.4</h3>
			<h4>Readme.html for OpenFeint iOS SDK 2.12.4<br>Release date 10.14.2011 <br>
				Release Notes Copyright (c) 2009-2011 OpenFeint Inc. <br>
				All Rights Reserved. 
			</h4>
			A complete set of the OpenFeint iOS 2.12.4 API Documentation can be seen at 
			<a href="http://support.openfeint.com/images/ref/2.12.4-OFSDK-iOS/APIdocumentation/index.html">http://support.openfeint.com/images/ref/2.12.4-OFSDK-iOS/APIdocumentation/index.html</a>.
			Note: A version of this README can also be downloaded <a href="http://support.openfeint.com/images/ref/2.12.4-OFSDK-iOS/OF2.12.4_APIdocumentation.zip">here</a>; the on-line version may have updated information not included below.
			<h3>&nbsp;&nbsp;In this document&nbsp;&nbsp;</h3>
			<ul>
				<li><a href="#new">&nbsp;&nbsp;New in OpenFeint iOS SDK Version 2.12.4&nbsp;&nbsp;</a>
				<li><a href="#req">&nbsp;&nbsp;Prerequisites</a>
				<li><a href="#qs">&nbsp;&nbsp;OpenFeint iOS Quick Start Guide&nbsp;&nbsp;</a>
				<li><a href="#gamefeed">&nbsp;&nbsp;GameFeed In the Sample App&nbsp;&nbsp;</a>
				<li><a href="#releasing">&nbsp;&nbsp;Releasing your title with OpenFeint&nbsp;&nbsp;</a>
				<li><a href="#gamefeed2">&nbsp;&nbsp;Displaying GameFeed In Your Game&nbsp;&nbsp;</a>
				<li><a href="#using">&nbsp;&nbsp;How To Use OpenFeint&nbsp;&nbsp;</a>
				<li><a href="#changelog">&nbsp;&nbsp;Changelog&nbsp;&nbsp;</a>
			</ul>
<a name="new"></a><h3>New in OpenFeint iOS SDK Version 2.12.4</h3>
<ul>
<li>OpenFeint OFSDK is now compatible with iOS 5.</li>
 </ul>

<a name="req"></a><h3>Prerequisites</h3>
<ul>
	<li>XCode version 3.2.2 or newer.
	<li>Build with Base SDK of 3.1.3 or newer.
</ul>
<h4>Privacy policy update</h4>
As an OpenFeint developer, please keep in mind that you must inform users of your applications:
<ul>
<li>Of all information you and OpenFeint will be collecting from your users and their devices and the uses for which you are collecting that information.</li>
<li>What information you will be providing to OpenFeint and what information OpenFeint will be collecting from your users and their devices and how OpenFeint may use that information -- even if your users do not opt in to the OpenFeint platform.</li>
<li>You should implement an appropriate privacy policy to address these and any other privacy-related issues.</li>
<li><a href="http://support.openfeint.com/gamefeed-information-collected/">Link to information collected</a></li>
</ul>
...
<a name="gamefeed"></a><h4>Notice That GameFeed Is Now Featured In the Sample App</h4>
GameFeed is a cross platform news ticker that features real-time updates from other players in their OpenFeint network.  
Using GameFeed, you can deliver news that is directly related to the game your players are playing, while they are playing it. 
...
<ol>
    ...
    <li><b>Important Notes About Installation</b>
        <ol>
	        ...
	        <li>Perform the "clean" operation. To do this, go to the Xcode menu bar and select <b>Product->Clean</b> from the top menu bar. This is to make sure no intermediate build products based on earlier versions of OpenFeint get incorporated into your newer builds. 
			<li>Make sure to download and install the latest offline configuration file from Developer Dashboard before you compile.</li>
			<li>OpenFeintPrefix.pch is gone.  You should not include it anymore:
			<br/>If you upgrade your version of OpenFeint and recompile, you will get an error - something like "Can't find OpenFeintPrefix.pch".  The solution is to delete the following line from your application's pch file:
			<pre>
			     #import "OpenFeint/OpenFeintPrefix.pch"</pre>
			</li>
        </ol>
...
Make sure that the following frameworks are included in your <b>Build Phases-&gt;Link Binaries With Libraries</b>: 
<ul> 
<li> 
<code>AddressBook</code> </li> 
<li> 
<code>AddressBookUI</code> </li> 
<li> 
<code>CFNetwork</code> </li> 
<li> 
<code>CoreGraphics</code> </li> 
<li> 
<code>CoreLocation</code> </li> 
<li> 
<code>CoreText</code> </li> 
<li> 
<code>Foundation</code> </li> 
<li> 
<code>GameKit</code> </li> 
<li> 
<code>libsqlite3.0.dylib</code> </li> 
<li> 
<code>libz.1.2.3.dylib</code> (unless using the compiler flag <code>OF_EXCLUDE_ZLIB</code> to turn off compression of high score and cloud storage data blobs.) </li> 
<li> 
<code>MapKit</code> </li> 
<li> 
<code>MobileCoreServices</code> </li> 
<li> 
<code>QuartzCore</code> </li> 
<li> 
<code>Security</code> </li> 
<li> 
<code>SystemConfiguration</code> </li> 
<li> 
<code>UIKit </code> </li> 
		</ul>
   ...
   (Xcode 3.22: do this by right clicking on your project and selecting <b>Add->Existing Frameworks...</b>)
	...
	<li><a name="gamefeed2"></a>Link GameFeed Into Your Game (optional, but we think it's a very good idea).  The easiest way to implement GameFeed is:
  	  <ul>
	    <li>At the top, add:
	      <pre>#import "OpenFeint/OFGameFeedView.h"</pre>
	    <li>To place the feed at the bottom of the screen, create an <code>OFGameFeedView</code> and add it to your main view, like this:
	      <pre>OFGameFeed *gameFeed = [OFGameFeedView gameFeedView];
   	      [self.view addSubview:gameFeed];
	      </pre>
		<li>If you require more control over the behavior of the feed, or would like to customize the look and feel, you may pass in a dictionary of settings 
		to <code>gameFeedViewWithSettings</code>. The settings are defined in <b>OFGameFeedSettings.h</b>. For example: 
			<pre>
	...
	<b>A note about using GameFeed with OpenGL</b><br>
	If you are using GameFeed on top of an OpenGL view, you may need some additional setup. 
	<br>By default, scrolling the GameFeed will cause your OpenGL animation to freeze. To fix this, change your code to allow your rendering code to run on all NSRunLoop "common modes".
	<br>For example, if you have code like this:
	<pre>
...
<br/> More information about each of these settings can be found below. These settings are also described in <b><a class="el" href="_open_feint_settings_8h.html">OpenFeintSettings.h</a></b>.<br/> 
<ul> 
<li> 
<code>OpenFeintSettingRequireAuthorization</code> - <em>deprecated</em>. </li> 
<li> 
<code>OpenFeintSettingDashboardOrientation</code> - Specifies orientation in which the OpenFeint dashboard will appear. </li> 
<li> 
<code>OpenFeintSettingShortDisplayName</code> - In certain areas where the application display name is too long, OpenFeint uses this more compact version of your application's display name. For example, this variable is used for the title of the current game tab in the OpenFeint dashboard. </li> 
<li> 
<code>OpenFeintSettingEnablePushNotifications</code> - Specifies whether or not your application will be enabling Push Notifications (for Social Challenges, currently). </li> 
</ul> 
<br/><b>About <code>OFDelegatesContainer</code> and <code>OpenFeintDelegate</code></b><br/> 
<code>OFDelegatesContainer</code> provides a way for you to specify all of the various delegates that OpenFeint features may require. <br/> 
<br/> 
If you are only using an <code>OpenFeintDelegate</code> you may use the simple convenience constructor: 
...
<hr>
<ul>
<li>OpenFeint's GameFeed features can now be used on iPad games.</li>
</ul>
<hr>
Version 2.12.2
<hr>
<ul>
<li>New Feature: GameFeed GameFeed is a cross platform news ticker that features real-time updates from other players in their OpenFeint network.  Using GameFeed, you can deliver news that is directly related to the game your players are playing, while they are playing it.   For more information, click <a href="http://support.openfeint.com/dev/of-2.12-intro/">here</a>.
<li>OpenFeint is taking steps to remove C++ from its SDK.  Please see the backwards compatibility layer for older APIs, but we encourage you to start to remove the C++ OpenFeint APIs from your code and move over to the new ObjC APIs.  For more information, click <a href="http://support.openfeint.com/dev/cmm-sum/">here</a>.
<li>OpenFeint now namespaces Facebook.  That means you have more control over customizing your own use of FBConnect.  You can now more easily create your own, independent FBConnect session.
<li>The OpenFeint Unity plugin is now removed from the iOS release.  For more information, please see <a href="http://support.openfeint.com/dev/ofunity-for-ios-release-notes/">OFUnity for iOS Release Notes</a>.
</ul>
<hr>
Version 2.10.1
<hr>
...
<ul>
	<li>Time-scoped Leaderboard view
		<ul>
			...
			<li>The compass/arrow icon in the upper right launches the map view
		</ul>
	<li>Out of Network Invites
			<p>Players can now choose from their Contact List and send invites via SMS or E-mail</p>
</ul>
<hr>
